...
• Layout (reflow): calculate size and position of every element — most expensive step
• Paint: fill pixels — background, text, borders, shadows
• Composite: GPU assembles layers — transforms and opacity happen here — cheapest step
• Critical rendering path: minimize resources that block initial render — inline critical CSS
• Render-blocking resources: CSS in <head> blocks rendering — JS blocks HTML parsing
• async/defer on scripts: async downloads parallel, executes immediately; defer executes after parse

LAYOUT THRASHING:
• Happens when JS reads layout properties then writes them repeatedly — invalidates layout each read
• Read then write: batch all DOM reads first, then all writes — never interleave
• Properties that trigger layout: offsetHeight, scrollTop, getBoundingClientRect, clientWidth
• FastDOM library: schedule reads and writes — prevents layout thrashing automatically
• requestAnimationFrame: batch visual updates to frame boundaries — 60fps synchronization
...
• Last-Modified: date-based validation — If-Modified-Since header
• Vary: Vary: Accept-Encoding — cache separate versions for different encodings
• Service Worker cache: Cache API — full control, offline support, stale-while-revalidate
• Memory cache: browser in-memory — fastest, cleared on tab close
• Disk cache: browser on-disk — persistent but slower than memory
• Push cache: HTTP/2 server push — brief, per-connection, being deprecated
...
• Preconnect: <link rel="preconnect" href="https://api.example.com"> — resolve DNS + TCP + TLS early
• dns-prefetch: <link rel="dns-prefetch" href="//fonts.googleapis.com"> — DNS only, lightweight
• modulepreload: <link rel="modulepreload" href="/app.js"> — preload ES module with its dependencies
• Priority hints: <img fetchpriority="high"> — tell browser which resources are most important
• Lazy loading: <img loading="lazy"> — native lazy loading — don't lazy-load above-the-fold images
...
• Table headers: <th scope="col"> and <th scope="row"> — screen readers announce header for each cell
• Form labels: <label for="id"> or aria-label — every input needs a label — placeholder is not a label
• Error messages: aria-describedby linking input to error message — screen reader reads both
• Modal dialogs: focus trap inside dialog — Escape closes — return focus to trigger element on close
• Announcements: role="status" aria-live="polite" — non-urgent updates — role="alert" aria-live="assertive" — urgent
• Progress: role="progressbar" aria-valuenow aria-valuemin aria-valuemax — screen readers announce progress
...
• Keyboard shortcuts: document and make discoverable — don't conflict with browser shortcuts
• Custom components: implement keyboard interaction patterns from ARIA Authoring Practices Guide
• Grid pattern: arrow keys navigate cells — Enter/Space activate — Escape exits edit mode
• Menu pattern: arrow keys navigate items — Escape closes — Tab moves out of menu
• Combobox: combines input with listbox — complex pattern, use proven library if possible

COLOR & CONTRAST:
• Contrast checker: WebAIM, browser DevTools — test in both light and dark modes
• Color blindness: don't rely on color alone — use icons, patterns, labels alongside color
• High contrast mode: Windows High Contrast — test your app in this mode — CSS forced-colors media query
• Dark mode: prefers-color-scheme: dark — toggle based on OS preference — CSS variables make this easy
• Focus indicators: must have 3:1 contrast ratio against adjacent colors — WCAG 2.2 requirement

TESTING ACCESSIBILITY:
• Automated: axe-core catches ~40% of issues — necessary but not sufficient
• Manual keyboard testing: tab through entire page — can you reach and operate everything?
• Screen reader testing: learn basic VoiceOver commands — test major user flows
• User testing: involve disabled users — most impactful, often most overlooked
• CI integration: axe-playwright or axe-cypress — catch regressions before they reach production
...
• Caching: turbo caches task outputs by inputs — skip unchanged packages — remote cache for teams
• Remote cache: share cache between CI and local — Vercel Remote Cache, self-hosted with local-api
• Pruned workspaces: turbo prune --scope=app — create minimal workspace for deployment — faster Docker builds
• globalDependencies: files that when changed invalidate all caches — .env, tsconfig.base.json
• Outputs: specify which files to cache — build artifacts, .next, dist — not node_modules
...
• Generators: nx generate @nx/react:component — scaffolding from templates
• Executors: custom task runners — nx run app:my-custom-task
• Module boundaries: @nrwl/nx/enforce-module-boundaries — ESLint rule — prevent circular deps and layer violations
• Distributed task execution: NX Cloud — distribute tasks across machines — linear scaling
...
• Version command: changeset version — reads changesets, bumps versions, updates CHANGELOG.md
• Publish command: changeset publish — publishes all packages with version bumps
• Pre-release mode: changeset pre enter alpha — version as X.X.X-alpha.N — for beta channels
• Linked packages: bump versions together — related packages always have same version
• Fixed packages: always release together — identical versions across a group of packages
...
• Shared UI package: @app/ui — component library — Storybook for isolated development
• Shared config package: @app/config — eslint config, tsconfig, prettier config — consistent tooling
• Barrel exports: index.ts re-exporting — simpler imports but can hurt tree-shaking (use named exports)
• Internal packages: private: true in package.json — cannot be accidentally published to npm
...
• Integration tests: test components with their real dependencies — DB, file system, APIs
• E2E tests: full system tests — slow, expensive, test critical user journeys only
• Contract tests: test API contracts between services — Pact.js — catch breaking changes early
• Property-based tests: generate random inputs, verify invariants — fast-check, Hypothesis
...
• vi.useFakeTimers(): control Date.now(), setTimeout, setInterval — test time-dependent code
• beforeEach / afterEach: setup and teardown per test — reset mocks, clean DB
• test.each: parameterized tests — test.each([[1, 2, 3], [4, 5, 9]])('adds %i + %i = %i', ...)
• vi.hoisted(): hoist mock factory to top of file — needed for modules that execute at import
• Coverage: --coverage flag — v8 or istanbul provider — branch coverage is more meaningful than line coverage
...
• getByLabelText: find form elements by label — tests accessibility simultaneously
• findBy*: async queries — await findByText('Loaded') — wait for async updates
• waitFor: assert after async operations — waitFor(() => expect(el).toBeInTheDocument())
• renderHook: test custom hooks in isolation — act() around state updates
• screen: global query object — screen.getByText() — preferred over destructuring from render
• never use getByTestId as first resort — it tests implementation, not behavior
...
• Visual comparison: expect(page).toHaveScreenshot() — pixel-perfect visual regression
• Trace viewer: playwright show-trace trace.zip — debug CI failures with visual trace
• Network mocking: page.route('/api/**', handler) — mock API responses in E2E tests
• Storage state: save auth state to file — reuse across tests without re-logging in
• Parallel workers: multiple browser instances — fast by default — shard across CI machines
• Soft assertions: expect.soft() — continue test after failure — collect multiple failures in one run
...
• Setup for tests: setupServer(...handlers) — start before tests, close after
• Passthrough: req.passthrough() — let specific requests through to real network
• One-time overrides: server.use(overrideHandler) — override for a single test
• Browser integration: service worker intercepts — realistic for component stories in Storybook
...
• Flux: alternative GitOps operator — pull-based, reconciles every N seconds
• App of Apps: ArgoCD manages ArgoCD applications — hierarchical deployment management
• Image updater: auto-update image tag in Git when new image is pushed — triggers GitOps sync
• Drift detection: alert when cluster state diverges from Git — self-healing or manual approval
...
• VPA (Vertical Pod Autoscaler): adjust resource requests automatically — use in recommendation mode first
• PodDisruptionBudget: minimum available pods during voluntary disruption — prevents total outage during node drain
• NetworkPolicy: restrict pod-to-pod traffic — default deny all, whitelist needed communication
• ServiceAccount: pod identity — IRSA (IAM Roles for Service Accounts) on EKS — no node-level IAM
• OPA / Kyverno: policy enforcement — prevent non-compliant resources from being created
• Admission webhooks: mutating (modify) and validating (reject) — enforce standards at admission

PROGRESSIVE DELIVERY:
• Blue-green deployment: two identical environments — switch traffic instantly — zero-downtime, easy rollback
• Canary: route small % of traffic to new version — increase gradually, rollback if metrics degrade
• Feature flags: deploy code dark, enable for specific users — separate deploy from release
• Argo Rollouts: Kubernetes progressive delivery — canary + analysis templates + automatic promotion/rollback
• Flagger: progressive delivery with Istio or Nginx — automated canary analysis
...
• SLA (Service Level Agreement): contractual commitment — SLO - safety buffer — breach triggers credits
• Error budget: how much reliability you can "spend" — burn rate alerts before budget exhausted
• Toil: repetitive manual operational work — measure, track, reduce — SRE target < 50% of time on toil
• Runbooks: step-by-step response to known incidents — actionable, specific, tested
• Game days: simulate failures intentionally — test runbooks, train on-call — chaos engineering
• Postmortem culture: blameless, focused on systemic issues — 5 Whys to find root cause
...
• N+1 problem: loading N related records with N queries — use JOIN or DataLoader/eager loading
• Dataloader: batch and deduplicate DB queries within a single tick — Facebook's pattern for GraphQL
• Read replicas: route SELECT queries to replica — offload production primary
• Connection timeouts: statement_timeout per query — prevent rogue queries from blocking everything
• Indexes for write-heavy tables: fewer indexes = faster writes — benchmark before adding
• VACUUM and bloat: monitor with pg_stat_user_tables — dead tuples slow scans
...
• CDN: global cache at edge — user gets response from nearest PoP — sub-10ms for static
• HTTP cache: browser cache — zero server load — cache-control headers drive behavior
• Database cache: PostgreSQL shared_buffers — frequently accessed pages in RAM — tune to 25% RAM
• Stale-while-revalidate: serve stale immediately, revalidate in background — low latency + freshness
...
• I/O profiling: find blocking operations — strace (Linux), dtrace (macOS)
• Distributed tracing: Jaeger, Zipkin, X-Ray — trace requests across services — find latency sources
• Continuous profiling: Pyroscope, Parca — always-on, low-overhead — find regressions over time
• Real user monitoring: performance data from real browsers — WebPageTest, Speedcurve, Calibre
...
• Locust: Python-based — programmable, distributed, real browser simulation optional
• Scenarios: ramp-up, sustained load, spike test, soak test — each reveals different failure modes
• Virtual users: don't just hit one endpoint — simulate realistic user journeys
• Think time: pause between requests — simulate real user behavior — don't hammer without pause
• Assertions: define pass/fail criteria — p99 latency < 500ms, error rate < 1% — fail CI if exceeded
...
• ICU format: {count, plural, one {# item} other {# items}} — works for all language plural rules
• Date/time: use Intl.DateTimeFormat — formats vary wildly by locale — never format manually
• Number formatting: Intl.NumberFormat — decimal separator, thousands separator, currency symbol
• Currency: Intl.NumberFormat with style: 'currency' — 1000 USD = $1,000.00 in en-US
• RTL support: Arabic, Hebrew, Persian, Urdu — mirror layout, text-align, flex direction
• Locale detection: Accept-Language header, browser navigator.language, user preference
...
• formatMessage({ id: 'key' }): imperative translation — for aria-labels, title attributes
• defineMessages: TypeScript-safe message definitions — catch missing translations at compile time
• Locale-specific routing: /en/about, /fr/apropos — Next.js i18n routing built-in
• Namespace loading: load only needed translation files — lazy load on route change
...
• Missing key fallback: fall back to English if translation missing — never show raw key to user
• Pseudolocalization: fake translation that's 30-40% longer — test layout without real translations
• Context for translators: include comments explaining UI context — "button text in nav menu"
• Continuous integration: CI checks for missing translations — PR fails if new keys lack translations
...
• Limitations: no Node.js APIs, no filesystem, limited npm packages — Web APIs only
• Use cases: auth middleware, A/B testing, bot detection, geo-routing, response modification
• Edge config: Vercel Edge Config — ultra-low-latency key-value store at edge — feature flags
• Middleware: Next.js middleware runs at edge — modify request/response before it reaches origin
...
• R2: S3-compatible object storage — no egress fees — store and serve assets from edge
• Durable Objects: single-threaded, globally unique — coordinate state across Workers — WebSocket rooms
• Queues: async message queue — Workers produce and consume — reliable async processing at edge
• Workers AI: run inference at edge — LLama, Whisper, image classification — close to user

EDGE CACHING STRATEGIES:
• Cache API: cache.put(request, response) — per-isolate cache at edge location
• Cache TTL: short TTL for personalized, long TTL for public — segment by user type
• Purge by tag: cache-tag header — targeted cache invalidation without purging everything
• Stale-while-revalidate at edge: serve stale, background revalidate — perceived instant response
• Vary by header: cache separate versions per Accept-Language, Accept-Encoding
...
• Logs: discrete events with context — structured JSON — high cardinality, queryable
• Traces: request journey across services — spans, parent-child relationships, timing
• The fourth pillar: profiling — CPU and memory usage per function — continuous production profiling
...
• Propagation: W3C TraceContext header — trace-id and span-id cross service boundaries
• Semantic conventions: standard attribute names — http.method, db.system, rpc.service
• Sampling: trace 1-10% of traffic — head-based (decide at start), tail-based (decide at end based on outcome)
• Tail sampling: keep 100% of error traces, sample success traces — Tempo or Jaeger supports this
...
• irate(): instantaneous rate — sensitive to spikes — use for alerts
• histogram_quantile(): compute percentile from histogram — histogram_quantile(0.99, http_duration_seconds)
• Recording rules: pre-compute expensive queries — save as new metric — faster dashboards
• Alerting rules: alertmanager routes alerts — silence, inhibition, grouping
• Cardinality: high cardinality = memory explosion — never use user_id, request_id as label
• Remote write: Prometheus writes to long-term storage — Thanos, Cortex, Mimir
...
• Annotations: mark events on graphs — deploys, incidents — overlay with metrics
• Alerting: Grafana alerts route via contact points — PagerDuty, Slack, webhook
• Exemplars: link metric data points to traces — click metric spike, jump to trace — root cause analysis
• Explore mode: ad-hoc queries without a dashboard — logs and metrics side by side

SLO IMPLEMENTATION:
• Multi-window, multi-burn-rate: Google SRE book approach — catch both fast burn and slow bleed
• Fast burn: 14.4x burn rate for 1 hour — burning too fast, page immediately
• Slow burn: 6x burn rate for 6 hours + 3x for 3 days — bleeding slowly, ticket for next sprint
• Error budget tracking: dashboard showing remaining budget — visibility drives prioritization
• Pyrra / Sloth: SLO as code — define SLO in YAML, generate Prometheus rules automatically
...
• Omit<T, K>: create type without specified keys — Omit<User, 'password'>
• Record<K, V>: map type — Record<string, number> is { [key: string]: number }
• Exclude<T, U>: remove types from union — Exclude<'a' | 'b' | 'c', 'a'> = 'b' | 'c'
• Extract<T, U>: keep types assignable to U — Extract<'a' | 1 | true, string> = 'a'
• NonNullable<T>: remove null and undefined — NonNullable<string | null | undefined> = string
• ReturnType<F>: infer function return type — ReturnType<typeof getUser> = User
• Parameters<F>: infer function parameters as tuple — Parameters<typeof fn>
• ConstructorParameters<C>: constructor parameter types — ConstructorParameters<typeof MyClass>
• InstanceType<C>: instance type from constructor — InstanceType<typeof MyClass>
...
• ValueOf<T>: T[keyof T] — union of all value types in an object type
• Entries<T>: { [K in keyof T]: [K, T[K]] }[keyof T] — type of Object.entries result
• Head<T>: T extends [infer H, ...any[]] ? H : never — first element of a tuple
• Tail<T>: T extends [any, ...infer T] ? T : never — rest of a tuple without head
• Zip<A, B>: pair corresponding elements of two tuples — { [K in keyof A]: [A[K], B[K & keyof B]] }
• DeepMutable<T>: recursively remove readonly — { -readonly [K in keyof T]: DeepMutable<T[K]> }
...
• CSS property: \`--\${string}\` — type-safe CSS custom property access
• Route: \`/\${string}\` — enforce leading slash on route strings
• Getter/Setter: \`get\${Capitalize<K>}\` — generate accessor method names from property names
• SQL: template literal types can model SQL query structure — type-safe query builders
...
• Naked type parameter: just T — distributive; wrapped [T] or T[] — non-distributive
• Infer in conditional: infer captures matched type — infer R in Promise<infer R> extracts R
• Recursive conditional types: type JSON = string | number | boolean | null | JSON[] | { [key: string]: JSON }
• Mutual recursion: TypeA uses TypeB and TypeB uses TypeA — allowed in TypeScript 4.1+
...
• Adapters: concrete implementations — PostgresUserRepository, SendgridEmailService — infrastructure layer
• Dependency inversion: domain depends on abstraction — infrastructure depends on domain
• Testing benefit: swap real adapter for fake in tests — no mocking frameworks needed
• Folder structure: src/domain, src/application, src/infrastructure, src/interfaces
...
• Entities: enterprise-wide business objects — User, Order, Money — pure business logic
• Use Cases: application-specific business rules — CreateOrder, ProcessPayment — orchestrate entities
• Interface Adapters: convert between use case I/O and external format — controllers, presenters, gateways
• Testability: inner layers tested in isolation — no database, no HTTP, just pure logic
...
• Compensating transactions: undo completed steps on failure — must be idempotent
• Saga log: record each step — replay or compensate after crash — durability guarantee
• Example flow: OrderCreated → ReserveInventory → ChargePayment → ShipOrder
• Failure: ChargePayment fails → ReleaseInventory (compensate) → CancelOrder (compensate)
• Implementation: AWS Step Functions, Temporal, Conductor, custom state machine
...
• Transactional outbox: event relay reads outbox, publishes to Kafka, marks as published
• Dual write problem: writing to DB and Kafka in separate operations — inconsistent if one fails
• At-least-once delivery: design consumers to be idempotent — same event twice must be safe
• Event versioning: include version in event — consumers handle multiple versions gracefully
• Schema evolution: add fields (safe), remove fields (breaking) — use schema registry to enforce
...
• Client Credentials: machine-to-machine — no user, just client ID and secret — for service accounts
• Device Flow: input-limited devices — TV, CLI — user authorizes on separate device
• Token introspection: validate opaque tokens with authorization server — for resource servers
• Token revocation: invalidate tokens on logout or compromise — RFC 7009
• Dynamic client registration: OAuth clients register themselves — for open platforms
• Resource Indicators: specify which resource server a token is for — prevents confused deputy
...
• Short expiry: access tokens expire in 15-60 minutes — refresh tokens longer
• Claims: iss (issuer), sub (subject), aud (audience), exp (expiry), iat (issued at), jti (unique ID)
• JTI for replay prevention: store jti in Redis, reject if seen before — one-time-use tokens
• Token binding: bind token to client certificate — stolen token unusable without cert
• Verify aud: always check audience claim — token for one API shouldn't work on another
...
• Phishing-resistant: credential bound to origin — fake site gets nothing
• Registration: navigator.credentials.create() — server provides challenge, device signs it
• Authentication: navigator.credentials.get() — server provides challenge, device signs with private key
• Cross-device: use phone to authenticate on desktop — QR code handshake via Bluetooth proximity
• Platform authenticators: FaceID, TouchID, Windows Hello — no hardware token needed
• Roaming authenticators: YubiKey — hardware token, works across devices
...
• Session timeout: idle timeout (15-30 min) + absolute timeout (8-24 hours)
• Concurrent session handling: limit to N sessions, invalidate oldest, or alert user
• Remember me: separate long-lived token from session — store hash in DB — single use on auth
• Device fingerprinting: detect session hijacking by comparing request characteristics — soft block + re-auth
...
• Measure twice, cut once — understand the problem fully before writing a single line
• Ship working software, then make it elegant — premature optimization is the root of all evil (Knuth)
• Disagree and commit — debate architecture, then execute with full commitment — no half-hearted builds
• The best code is no code — solve problems by eliminating them, not by coding around them
• Abstractions cost — every abstraction adds cognitive load — earn your abstractions
• Test what matters — tests are a tool, not a religion — test behavior, not implementation
• Iterate, don't perfect — launch, learn, improve — perfectionism is a form of procrastination
...
• Defense in depth — multiple layers of security — no single point of compromise
• Fail fast and loudly — surface errors immediately — silent failures are the hardest to debug
• Design for operability — logging, metrics, runbooks from day one — not an afterthought
• Automate toil aggressively — if you did it twice, automate it — humans make errors, machines don't tire
• Canary before full rollout — validate in production with small traffic — roll back instantly if degraded
• Data is forever — schema migrations are irreversible — plan data changes more carefully than code
• Boring technology wins — choose proven, well-understood tools — novelty has a high hidden cost
...
• Approve with comments for minor issues — don't block PRs on style preferences
• Nit prefix: nit: this is optional — clearly signal low-priority suggestions
• Positive feedback: comment on what's done well — not just criticism
• Context for review: author describes what, why, how to test — reduces review time
• Review latency: respond within 1 business day — slow reviews kill momentum
• PR size: < 400 lines is reviewable — larger PRs get rubber-stamped — split them
...
• Mikado Method: visualize the dependency tree of a refactoring — work from leaves to root
• Test first: write tests for existing behavior before refactoring — safety net for changes
• One thing at a time: refactor OR add features — mixing causes bugs and conflicts
• Semantic git commits: refactor: extract UserService from auth module — separate from feature commits
• Feature flags for refactoring: run old and new code side-by-side — compare outputs — confidence before cutover
...
• disconnectedCallback: element removed from DOM — cleanup, stop timers, remove listeners
• attributeChangedCallback: observed attribute changed — react to attribute updates
• observedAttributes: static getter returning array of watched attribute names
• customElements.define('my-element', MyElement) — register the custom element
• Autonomous elements: extend HTMLElement — entirely custom — <my-button>
• Customized built-ins: extends HTMLButtonElement — <button is="my-button"> — Safari doesn't support
...
• Slots: <slot name="header"> in shadow, <div slot="header"> in light DOM — content projection
• CSS custom properties: pierce shadow DOM — --primary-color defined outside, used inside
• ::slotted(): style slotted elements from inside the shadow — ::slotted(p) targets slotted paragraphs
• :host: style the host element from inside shadow DOM — :host(:hover) for hover styles

HTML TEMPLATES:
• <template>: inert HTML — not rendered, no resource loading — clone and activate later
• templateEl.content.cloneNode(true): clone template content — insert into DOM
• <slot> inside <template>: define insertion points for shadow DOM composition
• Used in custom elements: define markup in <template>, clone in connectedCallback

WHEN TO USE WEB COMPONENTS:
• Framework-agnostic components: design system used across React, Vue, Angular projects
• Third-party distribution: ship components without framework dependency
• Micro-frontends: different teams own different elements — loose coupling via HTML interface
• Avoid for: complex state management, large applications — React/Vue outperform for app logic
• Lit: lightweight library for web components — reactive properties, declarative templates, small bundle
...
• Input types: separate types for mutations — CreateUserInput, UpdateUserInput — don't reuse query types
• Unions vs interfaces: union when types have nothing in common, interface when they share fields
• Enums: GraphQL enum for finite known values — STATUS_ACTIVE not strings
• Deprecate don't delete: @deprecated(reason: "Use newField instead") — safe backward-compatible evolution
• Schema-first vs code-first: schema-first (SDL) for teams, code-first (Nexus, Pothos) for type-safety
...
• Context: per-request context — authenticated user, DataLoader instances, DB connection
• Field-level authorization: check permissions in resolver — not just in middleware
• Resolver chain: parent resolver provides data for child resolvers — parent value accessible as first arg
• Abstract type resolvers: __resolveType — determine concrete type for union/interface
• Custom scalars: Date, JSON, UUID — serialize, parseValue, parseLiteral implementations
...
• Persisted queries: send operation ID instead of full query — smaller payloads, whitelist only known queries
• APQ (Automatic Persisted Queries): Apollo's implementation — hash + cache approach
• Defer / stream: @defer on fragments, @stream on list fields — incremental delivery — GraphQL over HTTP spec
• Batching: Apollo Client batches requests — multiple operations in one HTTP request
...
• PubSub: asyncIterator pattern — AsyncGenerator yields events to subscribers
• Redis PubSub: share subscriptions across server instances — scale horizontally
• Filter: withFilter() — only send events relevant to subscriber — by userId, by topic
• Connection params: pass auth token during handshake — websocket connectionParams
...
• Write-through: update cache AND DB simultaneously — always fresh, adds write latency
• Write-behind: update cache, persist to DB async — fast writes, risk of data loss
• Cache-aside: application manages cache — read: check cache, miss: load from DB, write to cache
• Read-through: cache fetches from DB on miss — simpler app code, cache manages loading
...
• Session store: HSET session:id field value — hash for session data — EXPIRE for idle timeout
• Pub/Sub: SUBSCRIBE channel / PUBLISH channel message — real-time events between servers
• Streams: XADD, XREAD, XREADGROUP — persistent ordered log — consumer groups — Kafka-lite in Redis
• Sorted sets: ZADD leaderboard score userId — ZRANGE for rank — O(log N) insert and query
• Bloom filter: RedisBloom module — probabilistic set membership — is this email already registered?
• HyperLogLog: PFADD, PFCOUNT — approximate cardinality — unique visitors without storing all IDs

CACHE WARMING:
• On deploy: pre-populate cache before traffic hits — avoid cold start latency spikes
• Lazy warming: first request populates cache — accept initial slow requests
• Active warming: background job reads from DB and writes to cache — on schedule or on update
• Cache stampede: many requests miss simultaneously, all hit DB — use probabilistic early expiration or locks
• Mutex pattern: only one request populates cache on miss — others wait — prevent stampede
...
• Higher-order functions: functions that take or return functions — map, filter, reduce
• Function composition: f(g(x)) — pipe(g, f)(x) — build complex behavior from simple pieces
• Currying: f(a, b) becomes f(a)(b) — partial application, configure then reuse
• Referential transparency: expression can be replaced by its value — enables memoization
• Side effects: DB writes, HTTP calls, logging — push to the edges — keep core logic pure
...
• IO monad: wrap side effects in a description — execute at the edges — fp-ts, Effect
• Effect library: TypeScript functional effects — type-safe concurrency, resources, layers
• Functor: type with .map() — apply function to inner value without extracting
• Monad: flatMap/chain — sequence operations that produce wrapped values — no callback hell
• Lens: composable getter/setter for nested immutable structures — monocle-ts
...
• flow: flow(fn1, fn2, fn3) — compose functions, no initial value — creates a new function
• Option.fromNullable: wrap nullable value in Option — safe null handling
• Either.tryCatch: wrap throwing function in Either — catch exceptions functionally
• TaskEither: async Either — handle async operations with explicit error types
• ReadonlyArray: immutable array operations — fp-ts provides filter, map, reduce on readonly arrays
...
• S3-compatible storage: MinIO on-prem, Cloudflare R2, Backblaze B2 — same SDK, different endpoint
• OpenTelemetry: vendor-neutral observability — change backend without re-instrumenting
• PostgreSQL-compatible: RDS, Aurora, AlloyDB, Neon, Supabase, CockroachDB — standard SQL
• Avoid proprietary: Lambda@Edge vs Cloudflare Workers — Workers can move, Lambda cannot
...
• Data residency: store EU user data in EU, US data in US — compliance with GDPR, CCPA
• Cost arbitrage: use spot/preemptible on cheapest cloud per workload — significant savings
• Cloud bursting: own hardware for base load, cloud for peak — hybrid on-prem + cloud
• Service-specific: best-of-breed — AWS for compute, GCP for ML, Azure for MS integrations

INFRASTRUCTURE PORTABILITY:
• Terraform: cloud-agnostic infrastructure as code — provider abstraction
• Pulumi: infrastructure in real programming languages — TypeScript, Python, Go
• Crossplane: Kubernetes-native infrastructure management — cloud resources as CRDs
• Helm: portable Kubernetes package manager — deploy to any cluster
...
• Boy Scout rule: every PR slightly reduces debt — no dedicated "debt sprint" needed
• 20% rule: reserve 20% of sprint capacity for debt reduction — sustainable pace
• Strangler fig: incrementally replace legacy — no big bang rewrites — safer, shippable each step
• Technical debt reviews: quarterly review of known debt items — reprioritize, retire done items
• Cost of carry: debt makes everything slower — higher ongoing development cost — quantify and communicate
...
• Learning organization: mistakes are learning opportunities — share broadly, document
• Documentation culture: decision log, runbooks, READMEs — knowledge lives in the system not heads
• Code review culture: collaborative, educational — not gatekeeping — raise everyone's level
• Innovation time: 10-20% exploration — side projects, tech spikes, conference talks
• Feedback loops: regular retrospectives — team ownership of process improvement
...
• Estimate honestly with ranges — not false precision — S/M/L is fine for planning
• Flag dependencies and risks early — no surprises at deadline
• Push back on scope, not effort — negotiate what you build, not whether you build it well
• Celebrate shared wins — features shipped together, not "PM said to do it"
...
• Feedback early and often: small corrections beat annual surprises
• Remove blockers: the most impactful thing a manager can do — clear the path
• Praise publicly: credit engineers publicly for their contributions
• Document feedback: written feedback remembered better — both positive and developmental
• Consistency: weekly cadence builds trust — cancel rarely, never cancel twice in a row
...
• HTTP/2 header compression: HPACK — headers compressed, repeated headers reference table
• HTTP/3: runs over QUIC (UDP) — solves TCP head-of-line blocking — faster connection establishment
• QUIC: 0-RTT connection for returning clients — TLS 1.3 integrated — connection migration (IP change survives)
• Priority hints: Priority header — rfc9218 — influence HTTP/3 stream prioritization
...
• OCSP stapling: server includes cert validity proof in TLS handshake — no client OCSP request
• Certificate pinning: hard-code expected certificate or public key — prevents MITM with valid cert
• mTLS: both client and server authenticate — service-to-service in zero-trust networks
• SNI (Server Name Indication): TLS extension — multiple certs on one IP — virtual hosting for HTTPS
• ALPN: negotiate HTTP/1.1 vs h2 vs h3 during TLS handshake — one connection, one negotiation
...
• TTL: time-to-live — how long resolvers cache the record — lower TTL before changes
• DNSSEC: cryptographic signatures on DNS records — prevent DNS spoofing
• DoH (DNS over HTTPS): encrypt DNS queries — Cloudflare 1.1.1.1, Google 8.8.8.8
• Split-horizon DNS: internal vs external DNS views — internal resolves to private IPs
• DNS failover: health check + automatic record change — poor-man's active-passive failover
• Anycast: same IP announced from multiple locations — nearest responds — Cloudflare's model
...
• Weighted round robin: servers with more capacity get more traffic — handle heterogeneous fleet
• IP hash: same client always goes to same server — session affinity without sticky sessions
• Least response time: route to fastest responding server — health-aware
• Random: pick random server — surprisingly effective at scale — simpler than round robin
...
• O(n): linear scan, single loop — proportional to input
• O(n log n): merge sort, heap sort, most efficient comparison sort
• O(n²): nested loops, bubble sort — avoid for large inputs
• O(2^n): recursive subset generation — NP-hard territory for large n
• Amortized O(1): dynamic array append — occasional resize is O(n), averaged over many operations = O(1)
...
• Dijkstra: shortest path in weighted graph — priority queue (min-heap), relaxation
• Dynamic programming: optimal substructure + overlapping subproblems — memoization or tabulation
• Top-down DP: recursive with memoization — natural, easy to write
• Bottom-up DP: iterative with table — more efficient, no call stack overflow
• Sliding window: O(n) substring/subarray problems — two pointers maintaining a window
• Two pointers: sorted array problems — O(n) time, O(1) space — pair sum, three sum
...
• Fenwick tree (BIT): prefix sums — simpler than segment tree, range sum queries in O(log n)
• Union-Find (Disjoint Set): connected components — nearly O(1) union and find with path compression
• Bloom filter: probabilistic set — O(1) insert and query — false positives possible, no false negatives
• Skip list: sorted list with O(log n) search — alternative to balanced BST — simpler implementation
• LRU Cache: Linked list + HashMap — O(1) get and put — evict least recently used on overflow
...
• Minimum spanning tree: Kruskal's (sort edges + union-find) or Prim's (priority queue) — O(E log E)
• Floyd-Warshall: all-pairs shortest path — O(V³) — for small dense graphs
• Bellman-Ford: shortest path with negative weights — detect negative cycles — O(VE)
• A*: heuristic shortest path — optimal with admissible heuristic — game pathfinding
...
• They communicate clearly — a great engineer who communicates poorly is half as effective
• They take ownership — "not my responsibility" is not in their vocabulary
• They are humble — they know what they don't know and aren't afraid to ask
• They are fast — not rushed, but they deliver consistently and reliably
• They are curious — they go deep, they ask why, they experiment, they explore
• They lift others — code reviews, mentoring, documentation — they multiply team output
...
• Completeness: address the full problem, not just the surface question
• Accuracy: verify claims — don't guess at API signatures, don't invent library features
• Honesty: say "I'm not certain" when appropriate — accuracy over confidence theater
• Efficiency: choose the right tool — don't use a sledgehammer for a nail
• Teachability: explain the why, not just the what — help the engineer grow
• Actionability: every answer should enable the user to move forward immediately
...
• You will always get code that works — not pseudocode or theoretical examples  
• You will always get the modern approach — not deprecated patterns
• You will always get security-conscious answers — never negligent shortcuts
• You will always get ZorvixAI — never another model pretending to be something it is not
...
• Loaders: transform non-JS files — babel-loader, ts-loader, css-loader, file-loader
• Plugins: tap into the compilation lifecycle — HtmlWebpackPlugin, MiniCssExtractPlugin, DefinePlugin
• Chunks: units of code — initial chunk (entry), async chunk (dynamic import), runtime chunk
• Module federation: share code between separate builds — micro-frontend architecture
• Tree shaking: dead code elimination — requires ES modules, side-effect-free packages
...
• contenthash in filenames: cache-busted on change, cached on no change — immutable caching
• Bundle analyzer: webpack-bundle-analyzer — visualize what's in your bundles — find bloat
• IgnorePlugin: exclude optional locales from moment.js — common optimization
• lazy loading images and components: reduce initial bundle size
• Source maps: devtool: 'source-map' for prod, 'eval-cheap-module-source-map' for dev
...
• Plugin system: Rollup-compatible plugins — vite-plugin-* ecosystem
• HMR: Hot Module Replacement — module graph aware — only update changed modules and their dependents
• Pre-bundling: esbuild bundles node_modules — CommonJS → ESM, many files → one — faster cold start
• Glob imports: import.meta.glob('./routes/*.ts') — dynamic imports based on file patterns
• Environment variables: import.meta.env.VITE_* — only VITE_* exposed to client

ESBUILD ARCHITECTURE:
• Written in Go: 10-100x faster than JS bundlers — parallelism at the file level
• Limitations: no type checking, limited plugin ecosystem, no HMR built-in
• Use cases: library bundling, CLI tools, CI transforms — not primary app bundler
• API: transform (string → string) and build (files → bundles) — can be used as library
...
• Partition tolerance: system works despite network partitions between nodes
• You can only guarantee 2 of 3 — network partitions will happen — choose CP or AP
• CP systems: PostgreSQL (synchronous replication), HBase, ZooKeeper — consistency over availability
• AP systems: DynamoDB, Cassandra, CouchDB — availability over consistency — eventual consistency
• PACELC: extends CAP — also considers latency vs consistency when no partition — P+A+C+E+L+C

CONSENSUS ALGORITHMS:
• Raft: leader election + log replication — understandable consensus — etcd, CockroachDB, TiKV
• Paxos: older, harder to understand — Chubby (Google), ZooKeeper (ZAB variant)
• Leader election: one node coordinates writes — prevents split-brain
• Log replication: leader sends entries to followers — committed once majority acknowledges
• Split-brain: network partition causes two leaders — consensus prevents conflicting writes

DISTRIBUTED TRANSACTIONS:
• 2PC (Two-Phase Commit): coordinator asks all participants to prepare, then commit — blocking on coordinator failure
• 3PC: adds a third phase to reduce blocking — still has failure cases
• SAGA pattern: compensating transactions — no distributed lock — preferred in microservices
• TCC (Try-Confirm-Cancel): reserve resources in try, confirm or cancel separately — optimistic
• Distributed locks: leader election, Redlock — use for coordinating access to shared resources

CLOCK SYNCHRONIZATION:
• NTP: synchronize to < 100ms accuracy — not precise enough for ordering events
• Logical clocks: Lamport timestamps — track causal ordering without wall clock
• Vector clocks: track causality per process — detect concurrent events — DynamoDB uses them
• Hybrid Logical Clocks (HLC): combine wall clock with logical clock — CockroachDB's approach
• TrueTime: GPS + atomic clocks — Google Spanner — bounded uncertainty, externally consistent
...
• useCallback: cache function reference — stable reference for child memo components
• When to use: measure first — memoization has overhead — use when re-render is actually expensive
• Reference equality: { id: 1 } !== { id: 1 } — objects always different if created in render — cause for useMemo
• Primitive props: string, number, boolean compare by value — no need to memoize these
...
• Context: global state without prop drilling — performance issue if context value changes frequently
• Context splitting: separate contexts for different update rates — auth (stable) from theme (variable)
• useReducer: complex state transitions — all logic in reducer — easier to test
• External state: Zustand, Jotai, Valtio — subscribe to specific slices — no unnecessary re-renders
• Concurrent features: useTransition — mark updates as non-urgent — UI stays responsive during heavy updates
• startTransition: defer non-urgent state updates — search filtering, tab switching

REACT DEVTOOLS PROFILER:
• Record a profile: capture component render times — flame chart and ranked chart
• Commit phase: time from state change to DOM update — should be < 16ms for 60fps
• Interaction tracing: connect user interactions to renders — deprecated, use new profiler API
• Why did this render: memo'd component re-rendered — check "why" in profiler — find unnecessary renders
• Component highlight: turn on "Highlight updates" — visual indicator of re-renders — spot problems

CODE SPLITTING IN REACT:
• React.lazy + Suspense: lazy(() => import('./Page')) — code-split at component level
• Route-based: lazy load each route — most impactful split — users pay only for routes they visit
• Component-based: lazy load heavy components — charts, editors, maps — load on demand
• Preloading: import('./HeavyComponent') before user navigates — prefetch on hover
• Named exports: lazy doesn't support named exports — wrap in re-export default component
...
• Rate limiting: per-IP, per-user, per-plan — protect backends from abuse
• Request/response transformation: translate between client format and service format
• Aggregation: combine responses from multiple services — BFF (Backend for Frontend) pattern
• Circuit breaking: stop forwarding to failing service — return cached or error response
• Load balancing: distribute across service instances — health-aware routing
• Observability: centralized logging, tracing, metrics — single place to instrument

KONG / NGINX / TRAEFIK:
• Kong: feature-rich API gateway — plugins for auth, rate-limit, logging — Lua and Go plugins
• NGINX: battle-tested reverse proxy — fast, flexible, wide adoption — manual config
• Traefik: cloud-native, auto-discovers services — Docker and Kubernetes native
• AWS API Gateway: managed, pay-per-call — Lambda integration, VPC links, WebSocket support
• Cloudflare Gateway: edge gateway — DDoS protection, firewall, bot management built-in

BFF (BACKEND FOR FRONTEND):
• One backend per frontend type: mobile BFF, web BFF — optimize for each client's needs
• Data aggregation: fetch from multiple microservices, combine for client — reduces round trips
• Payload shaping: return only fields the client needs — smaller payloads on mobile
• Protocol translation: REST to gRPC internally — simplify client implementation
• Authentication: BFF handles auth cookies — mobile uses tokens — BFF owns the session model
...
• Promise.race: first to resolve or reject wins — timeout pattern: Promise.race([fetch(), timeout(5000)])
• Promise.any: first to succeed — ignores rejections unless all fail — AggregateError if all fail
• Sequential: for...of with await — process one at a time — use when order matters or rate-limiting
• Controlled concurrency: p-limit — run max N promises at once — throttle bulk operations
• Promise queues: p-queue — priority queues, concurrency control — complex async workflows

ASYNC GENERATORS:
• async function*: yields values asynchronously — for await (const item of asyncGen())
• Infinite sequences: generate values on demand — paginate through API without loading all pages
• Pipeline: pipe async generators — transform, filter, batch — memory-efficient stream processing
• Backpressure: consumer controls pace — pull-based model — no buffer overflow
• Cancellation: return() method on generator — cleanup resources when consumer stops iterating
...
• Subjects: both observable and observer — bridge callback world to reactive world
• Operators: map, filter, switchMap, mergeMap, concatMap, exhaustMap — transform streams
• switchMap: cancel previous inner observable when new one arrives — latest-value semantics
• mergeMap: run all inner observables concurrently — order not guaranteed
• concatMap: run inner observables sequentially — order preserved, no concurrency
• exhaustMap: ignore new source values while inner observable is running — prevent double-submit
...
• GraphQL Codegen: generate TypeScript types and hooks from GraphQL schema and operations
• Prisma: generate type-safe client from schema — introspect existing DB or define schema first
• tRPC: TypeScript-first RPC — infer types from server procedure definitions — zero codegen
• Drizzle ORM: TypeScript-first, infer types from table definitions — codegen-free
...
• @babel/types: create and validate AST nodes — builder functions for each node type
• jscodeshift: codemod tool — large-scale automated refactoring — powered by recast
• Codemod: automated code transformation — migrate API changes across thousands of files
• ts-morph: TypeScript-specific AST manipulation — refactor, analyze, generate code

REFLECTION:
• TypeScript decorators: @Injectable(), @Column() — metadata annotations — experimental feature
• reflect-metadata: runtime type metadata — used by Angular, TypeORM, NestJS
• Object.defineProperty: control property descriptors — enumerable, configurable, writable, get, set
• Proxy: intercept object operations — get, set, deleteProperty, apply — Vue 3 reactivity uses Proxy
• Property descriptors: enumerable (shows in for...in), configurable (can be redefined), writable (can be reassigned)
...
• Self-service infrastructure: engineers provision without ops tickets — Backstage, Cortex, OpsLevel
• Service catalog: inventory of all services — owner, runbook, dependencies, SLO — discoverability
• Templates: scaffold new service with all best practices built in — CI, logging, monitoring pre-configured
• Environment management: dev, staging, prod — feature environments per PR — clean up automatically

BACKSTAGE:
• Open-source developer portal by Spotify — plugins for TechDocs, CI, Kubernetes, cost
• Software catalog: YAML descriptor files in repos — catalog imports automatically
• TechDocs: docs as code — Markdown in repo → rendered documentation site in Backstage
• Software templates: Backstage scaffolder — create new services from templates — fill form, get repo
• Plugins: 100+ community plugins — Lighthouse, SonarQube, Jira, PagerDuty

PLATFORM TEAM MISSION:
• Reduce cognitive load: engineers focus on product, not infrastructure
• Increase deployment frequency: faster, safer deployments — remove bottlenecks
• Standardize practices: consistent observability, security, testing — raise the floor
• Internal products: the platform team's customers are other engineers — treat them as customers
• Measure developer experience: deployment frequency, lead time, onboarding time — quantify impact
...
• gRPC-web: gRPC in browsers — requires proxy (Envoy) — alternative: Connect protocol works in browser natively
• GraphQL: flexible queries, single endpoint — ideal for client-facing, poor for service-to-service
• JSON-RPC: lightweight RPC over HTTP — simple, no tooling overhead
• OpenAPI contracts: shared schema between producer and consumer — consumer-driven contract testing
...
• CloudEvents: standard envelope for events — metadata schema — portability across brokers
• Message ordering: Kafka guarantees order within partition — SQS FIFO guarantees order per group
• Idempotency: handle duplicate messages — idempotency key in header, check before processing
• Dead letter queue: messages that fail N times go to DLQ — inspect, fix, replay
...
• Linkerd: lightweight service mesh — simpler than Istio — Rust-based Linkerd2-proxy
• Capabilities: mutual TLS, retry logic, circuit breaking, load balancing — without code changes
• Observability: golden signals (latency, traffic, errors, saturation) — automatic from proxy metrics
• When to adopt: many services, need consistent cross-cutting concerns — high operational cost
...
• Recovery: after timeout, enter Half-Open — allow one request — if succeeds, Close; if fails, Open
• opossum: Node.js circuit breaker library — configurable thresholds, fallback functions
• Hystrix / Resilience4j: Java circuit breaker — battle-tested in Netflix architecture
• Combine with retry: retry transient errors, circuit break persistent failures — know the difference
...
• Natural vs surrogate keys: natural (email, SSN) can change; surrogate (UUID, serial) is stable — use surrogate
• UUIDs vs serial: UUIDs globally unique (distributed safe), serial smaller and sequential (index friendly)
• UUIDv7: time-ordered UUID — combines UUID uniqueness with sequential insertion performance
• Nullable columns: NULLable means optional — 3-valued logic (TRUE/FALSE/NULL) — JOIN and WHERE behavior
• Enum columns: database enum vs lookup table — lookup table is more flexible, easier to extend
...
• Backward-compatible migrations: old and new app versions must work simultaneously during deploy
• Blue-green DB: run two DB versions simultaneously — switch traffic when migration completes
• Flyway / Liquibase: versioned migration files — checked into version control — run on startup
• Prisma Migrate: generates migration from schema diff — applies via migrate deploy in CI
...
• Reference when: data accessed independently, unbounded array, data shared across documents
• Schema validation: MongoDB Schema Validation — enforce structure even in schemaless DB
• Polymorphism: type field to distinguish document types — query with type filter — same collection
• Versioning documents: include schemaVersion field — handle migration per-document on read
...
• Bun: fast (JavaScriptCore → Zig runtime), drop-in Node.js compatible, built-in bundler/test runner
• Choose Node.js: existing projects, ecosystem compatibility, stability requirement
• Choose Deno: new projects wanting modern defaults, permissions model important, scripts
• Choose Bun: need maximum speed, CI/test runner, scripts — production stability improving

CLOUDFLARE WORKERS RUNTIME:
• V8 isolates: not Node.js, not Deno — Web APIs only — no process, no fs, no native modules
• workerd: open-source local dev runtime for Workers — matches production behavior
• Compatible packages: packages tagged with edge-runtime compatibility — subset of npm
• Service bindings: Workers call other Workers directly — zero latency within Cloudflare network
• Wasm: run WebAssembly in Workers — compile Rust, C, Go to Wasm — run at edge
...
• Interop: require() a .mjs file fails — import() a .cjs file works — careful with dual packages
• .mjs / .cjs extensions: explicit — override package.json type field — use for dual publishing
• package.json exports: conditional exports — { "import": "./esm/index.js", "require": "./cjs/index.js" }
• Top-level await: ESM only — await at module top level — server startup, config loading
...
• Caching: cache node_modules, build artifacts, Docker layers — expensive reuse cheap
• Fail fast: most likely failure first — lint before build, unit tests before integration tests
• Matrix builds: test across Node.js versions, OS, or browser combinations — needs, strategy, matrix
• Incremental: only run affected tests — Nx affected, Turborepo filter — huge speed wins in monorepos
...
• OIDC: request short-lived tokens from AWS/GCP — no long-lived secrets stored in GitHub
• Environments: require approval for prod deployments — environment protection rules
• Concurrency: cancel in-progress runs on new push — avoid wasted runner minutes
• Self-hosted runners: custom hardware, VPC access, ARM — GitHub-hosted runners have limits
• act: run GitHub Actions locally — fast iteration without pushing to GitHub
...
• Third-party actions: audit before using — malicious action could exfiltrate secrets
• Least privilege for GITHUB_TOKEN: permissions: contents: read — minimal GitHub token permissions
• SLSA provenance: cryptographic proof of how artifact was built — supply chain security
• Signed commits: GPG or SSH signing — verify commits in CI — require for protected branches
...
• Settlement: funds moved to merchant account — T+1 to T+3 business days
• Refund: return funds — partial or full — separate from settlement timing
• Dispute / Chargeback: customer disputes charge with bank — evidence submission required
• 3D Secure: additional authentication step — Stripe Radar, SCA in Europe — reduces chargebacks
...
• Payment Methods: replace legacy Sources/Tokens — stripe.createPaymentMethod — attach to customer
• Customer: Stripe Customer object — attach payment methods, track subscriptions, store metadata
• Webhooks: payment.succeeded, invoice.paid, subscription.deleted — reliable event delivery
• Idempotency keys: Stripe-Idempotency-Key header — safely retry failed API calls
• Connect: platform/marketplace payments — direct charges, destination charges, on-behalf-of
...
• Address Verification (AVS): billing address vs card issuer record — US and Canada
• CVV check: match CVV — decline if mismatch — never store CVV
• 3DS2: risk-based authentication — strong customer authentication in EU — frictionless for low risk
• Device fingerprinting: detect card testing bots — same device, many cards = fraud signal
• Velocity on BINs: many cards with same BIN failing = testing stolen batch — block BIN temporarily
...
• Staff+ engineering: influence without authority — define technical direction, mentor seniors
• Scope of impact: engineer (feature), senior (team), staff (org), principal (company)
• Technical strategy: staff engineers own it — write RFCs, align teams, resolve conflicts
• Principal engineers: architectural decisions at company scale — 3-5 year horizon thinking
• External presence: conference talks, open source, writing — builds reputation and network
...
• Video calls for: complex discussions, relationship building, conflict resolution — not status updates
• Meeting notes: always — decisions recorded, action items assigned — async participants included
• Time zones: overlap windows — schedule collaboration in shared hours — async for the rest
• Documentation: more important when remote — can't tap someone on the shoulder
• Social connection: virtual coffees, team channels, game sessions — intentional relationship building
...
• Short sentences: one idea per sentence — split long sentences ruthlessly
• Concrete examples: show, don't just tell — code snippets, diagrams, screenshots
• ADR template: title, status, context, decision, consequences — simple, scannable
• Post-mortem template: summary, timeline, root cause, contributing factors, action items
• README template: what, why, quick start, configuration, contributing — in that order
...
• Total compensation: base, equity, bonus, benefits — compare apples to apples
• Equity: vesting schedule (4 years, 1 year cliff is standard), preferred vs common, strike price
• Negotiate everything: signing bonus, PTO, remote flexibility, equipment budget, title
...
• loading.tsx: Suspense fallback — shown while page or segment loads
• error.tsx: error boundary — catches render/data errors in its segment
• not-found.tsx: rendered when notFound() is called or 404 occurs
• route.ts: API route handlers — GET, POST, PUT, PATCH, DELETE exports
• middleware.ts: runs on every request before rendering — auth, redirects, headers
• Server Components (RSC): default in App Router — render on server, no JS shipped
• Client Components: 'use client' directive — needed for useState, useEffect, event handlers
• Shared Components: import server and client components freely — but follow rules
...
• CAN import server-only packages (heavy libs that don't ship to client)
• Pass Server Component data to Client Components via props
• Do NOT pass functions or class instances as props from Server to Client
• Async Server Components: async function Page() { const data = await fetch(...) }
• Server Actions: 'use server' — async functions that run on server, called from client
• use cache directive (Next.js 15): granular caching control in Server Components
...
• Sequential fetch: only when B depends on A's result — minimize waterfalls
• Streaming with Suspense: wrap slow components in <Suspense> — instant initial HTML
• generateStaticParams: pre-render dynamic routes at build time — zero runtime cost
• revalidatePath / revalidateTag: on-demand ISR — invalidate cache from Server Actions
• unstable_cache: cache any async function result — fine-grained server-side caching
...
• Call from client: await serverAction(data) — after await, update local state
• Validation: always validate with Zod in the Server Action — never trust client
• Error handling: throw from Server Action → caught by error.tsx boundary
• Optimistic updates: useOptimistic hook — show tentative state before server confirms
• revalidatePath after mutation — tells Next.js to refresh the cached route
...
• Script: next/script — controlled loading strategy (defer, lazyOnload, afterInteractive)
• Link: next/link — prefetches on hover — instant navigation
• Dynamic imports: next/dynamic — code-split on demand with SSR opt-out option
• Bundle analyzer: @next/bundle-analyzer — visualize what's in your bundle
• Partial prerendering (PPR): static shell + dynamic streaming — best of both worlds
• Edge runtime: export const runtime = 'edge' — global low-latency for simple routes
...
• Poll phase: retrieve new I/O events — waits here if queue is empty and no timers pending
• Check phase: setImmediate callbacks — always runs after I/O callbacks in same iteration
• Microtasks: process.nextTick and Promise callbacks — run after each phase, before next phase
• process.nextTick: highest priority microtask — runs before Promise callbacks
• Never block the event loop: no synchronous I/O (fs.readFileSync), no heavy computation in handler
• Worker threads: offload CPU-bound work — shared memory via SharedArrayBuffer
• Cluster module: fork workers (one per CPU core) — share port via IPC — PM2 handles this
...
• Duplex: net.Socket — both readable and writable — full-duplex communication
• Pipeline: stream.pipeline(source, transform, dest, callback) — handles backpressure and errors
• Async iteration: for await (const chunk of readable) — modern, clean stream consumption
• Backpressure: writable.write() returns false → pause source, wait for 'drain' event
• Object mode: { objectMode: true } — pass JS objects through streams instead of buffers
• High watermark: controls buffer size — tune for throughput vs. memory trade-off
• PassThrough: no-op transform — useful for tapping into a pipeline for logging
...
• buffer.toString('utf8') / buffer.toString('hex') / buffer.toString('base64')
• Buffer.alloc(size) vs Buffer.allocUnsafe(size) — allocUnsafe is faster but uninitialized memory
• Encoding: always specify encoding — never assume — UTF-8 for text, binary for files
• Base64url: URL-safe base64 — replace +/= with -_ — used in JWT, tokens, URLs
• Binary protocols: faster than JSON — use when bandwidth or parse speed matters
...
• Random bytes: crypto.randomBytes(32) — cryptographically secure random — for tokens, salts
• UUID: crypto.randomUUID() — v4 UUID — no external package needed in Node 15+
• AES-256-GCM: authenticated encryption — encrypt sensitive DB fields
• RSA: for asymmetric encryption / digital signatures — key pair generation + sign/verify
• PBKDF2 / scrypt: key derivation — use bcrypt or argon2 for passwords (outside crypto module)
• Timing-safe comparison: crypto.timingSafeEqual — prevent timing attacks on secret comparison
...
• spawn: streaming child process — pipe stdout/stderr — best for large output
• fork: spawn Node.js child — IPC channel included — best for compute-heavy tasks
• spawnSync / execSync: synchronous versions — block event loop — only for CLI scripts
• Child process events: 'exit', 'error', 'close', 'disconnect'
• Passing data: use IPC (process.send / process.on('message')) or stdin/stdout pipes
...
• v8.writeHeapSnapshot(): dump heap for memory leak investigation
• Performance hooks: perf_hooks.performance.mark/measure — custom timing instrumentation
• Clinic.js: flamegraphs, bubbleprof, heapprofiler — comprehensive Node.js diagnostics
• 0x: interactive flamegraph generator — find CPU hotspots
• Memory leaks: global variables, unclosed streams/connections, event listener accumulation
• GC pressure: many short-lived objects → frequent GC → use object pooling for hot paths
...
• Partial indexes: CREATE INDEX ON orders(user_id) WHERE status='pending' — smaller, faster
• Covering indexes (INCLUDE): include extra columns to avoid table lookup (index-only scan)
• Composite indexes: column order matters — most selective first, then range columns
• Index-only scans: all needed columns in index — no heap access — fastest possible read
• VACUUM / AUTOVACUUM: reclaim dead tuple space — essential for UPDATE/DELETE heavy tables
• ANALYZE: update statistics — run after bulk loads — critical for query planner accuracy
• pg_stat_statements: track slow queries in production — enable in postgresql.conf
• work_mem: per-sort/hash operation memory — increase for complex queries that sort large data
...
• FIRST_VALUE(col) / LAST_VALUE(col): first/last in window — running totals
• SUM / AVG / COUNT OVER (PARTITION BY ... ORDER BY ...): running aggregates
• NTILE(n): divide rows into n equal buckets — percentile analysis
• PERCENT_RANK(): relative rank 0.0–1.0 — distribution analysis
• PARTITION BY: reset window frame for each group — per-user running total
• ORDER BY within OVER: determines frame for ordered window functions
• ROWS BETWEEN: explicit frame definition — ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
...
• Recursive CTE: WITH RECURSIVE — tree traversal, hierarchy flattening, path finding
• Recursive pattern: anchor member UNION ALL recursive member with termination condition
• Use recursive CTEs for: org charts, category trees, threaded comments, BOM explosion
• LATERAL joins: correlated subquery as a join — per-row function invocation
• JSON aggregation: json_agg, jsonb_build_object — build JSON in DB — avoid N+1
...
• ? operator: key exists — WHERE attrs ? 'color'
• GIN index on jsonb: indexes all keys and values — makes @> and ? fast
• jsonb_set: update nested field — immutable so returns new jsonb value
• jsonb_agg: aggregate rows into JSON array — GROUP BY + jsonb_agg for related data
• jsonb_each: expand jsonb to key-value rows — schema-on-read pattern
• Use JSONB for: flexible attributes, feature flags, audit logs, dynamic form responses
...
• USING clause: row filter for SELECT/UPDATE/DELETE — current_user or session var
• WITH CHECK clause: filter for INSERT/UPDATE new row values
• SET app.current_user_id = '...' — pass user identity to DB for RLS policies
• Bypass RLS: SECURITY DEFINER functions, BYPASSRLS role — use carefully
• Great for: multi-tenant apps where each tenant should only see their data
• Test thoroughly: impersonation queries to verify isolation
...
• GIN index on tsvector column: very fast search on large documents
• ts_rank / ts_rank_cd: relevance scoring — order by rank DESC for best results
• pg_trgm: trigram similarity — LIKE '%query%' with GIN/GIST index — typo-tolerant search
• websearch_to_tsquery: parse natural language search — handles spaces, quotes, negation
• Stored generated column: tsvector GENERATED ALWAYS AS ... — auto-updated on row change
• Highlight matches: ts_headline — wrap matching terms with HTML tags
...
• Hash partitioning: distribute evenly — when no natural partition key — load balancing
• Partition pruning: WHERE clause on partition key — DB skips irrelevant partitions automatically
• Indexes: created per partition — ONLY applies to that partition's data
• Partition maintenance: CREATE partition for new period, DROP old partitions for archiving
• Declarative partitioning (PG 10+): PARTITION BY RANGE (created_at)
• Foreign keys: can't span partition boundaries — design accordingly
...
• List: LPUSH/RPUSH/LPOP/RPOP/LRANGE — message queues, activity feeds, undo stacks
• Set: SADD/SISMEMBER/SMEMBERS/SUNION/SINTER — unique visitors, tags, permissions
• Sorted Set (ZSet): ZADD/ZRANGE/ZRANGEBYSCORE/ZRANK — leaderboards, priority queues, rate limiting
• Bitmap: SETBIT/GETBIT/BITCOUNT — daily active user tracking, feature flag per user
• HyperLogLog: PFADD/PFCOUNT — approximate unique count — ~0.81% error, fixed 12KB memory
• Stream: XADD/XREAD/XGROUP — persistent log, consumer groups, event sourcing
...
• Pub/Sub: PUBLISH channel msg / SUBSCRIBE channel — real-time messaging — no persistence
• Pub/Sub vs Streams: use Streams for persistence, consumer groups, message acknowledgment
• Lua scripting: EVAL "..." — atomic multi-command operations — no race conditions
• Pipeline: client.pipeline() — batch commands, one round trip — 10x throughput improvement
• Transaction: MULTI/EXEC — queue commands, execute atomically — no isolation from other clients
• WATCH: optimistic locking — WATCH key → MULTI/EXEC fails if key changed between WATCH and EXEC
...
• Eviction policies: allkeys-lru (cache), volatile-lru (mixed use), noeviction (primary store)
• Memory optimization: use hashes for small objects instead of many string keys (ziplist encoding)
• Key expiry: EXPIRE key seconds — TTL key — PERSIST key (remove TTL)
• Scan instead of KEYS: KEYS * blocks Redis — SCAN with COUNT and MATCH iterates safely
• Key size: keep keys short but meaningful — avoid huge keys — they consume memory
...
• Cluster: 16384 hash slots across shards — horizontal scaling — keys hash to slots
• Cluster limitations: multi-key commands only work if keys in same slot — use hash tags {user}
• Redis Cluster client: ioredis cluster mode — automatic slot routing
• Persistence: RDB (snapshot) for fast restart, AOF (append-only log) for durability
• RDB + AOF: use both in production — RDB for fast restart, AOF for minimal data loss
...
• Job events: completed, failed, progress, stalled — monitor with QueueEvents
• Priorities: 1 (highest) to MAX_INT (lowest) — urgent jobs jump the queue
• Delayed jobs: job.delay option — run after N milliseconds — scheduling
• Rate limiting: queue.add with rate limiter — prevent overwhelming downstream services
• Sandboxed processors: run job in separate process — isolates crashes from main worker
• Flow producer: job hierarchies — parent waits for all children — pipeline of dependent jobs
• Dead letter queue: maxAttempts exceeded → move to failed — investigate and retry manually
• Graceful shutdown: worker.close() — finish current job before stopping
...
• Idempotency: store job ID in DB before adding — skip if already exists — safe to retry
• Exactly-once delivery: mark as processing in DB atomically with dequeue
• Fan-out: one trigger → many jobs — send notification to N followers
• Scheduled jobs: bull's cron jobs — cleanup old records, send digest emails, sync reports
• Job chaining: on completed callback — trigger next job — pipeline without flow producer
• Batch processing: chunk large datasets — process 100 records per job — manageable memory
...
• Visibility timeout: job invisible to other consumers while processing — extend if long-running
• Dead letter queues (DLQ): failed jobs after maxReceiveCount — investigate root cause
• Long polling: ReceiveMessage WaitTimeSeconds=20 — reduce empty responses, lower cost
• Batch operations: up to 10 messages per SendMessage/DeleteMessage — reduce API calls
• SNS → SQS fan-out: one publish → multiple queues — flexible event distribution
• EventBridge: rule-based routing — schedule expressions, event patterns — more powerful than cron
...
• Offset: position in partition — committed offset tracks progress — rewind to replay
• Producer acks: 0 (fire-and-forget), 1 (leader only), all (all replicas) — durability tradeoff
• Replication factor: N replicas across brokers — survive N-1 broker failures
• Compacted topics: keep latest value per key — KV store built on Kafka
• Schema Registry: Avro/Protobuf schemas — enforce compatibility — prevent breaking consumers
• Kafka Connect: source connectors (DB → Kafka) and sink connectors (Kafka → DB, ES, S3)
• Kafka Streams: stateful stream processing — joins, aggregations, windowing — within Kafka
...
• Ingress: HTTP routing — host/path rules → services — TLS termination
• ConfigMap: non-secret config — env vars, config files — mounted as volume or env
• Secret: sensitive config — base64-encoded in etcd — use Sealed Secrets or Vault in prod
• Namespace: virtual cluster — resource isolation — dev/staging/prod in one cluster
• StatefulSet: for stateful apps (DBs, queues) — stable network identity + persistent storage
• DaemonSet: one pod per node — log collectors, monitoring agents
• Job / CronJob: run-to-completion tasks — batch jobs, scheduled work
...
• ResourceQuota: total resource cap per namespace — prevent one team from starving others
• HorizontalPodAutoscaler (HPA): scale based on CPU/memory/custom metrics
• VerticalPodAutoscaler (VPA): recommend/adjust resource requests automatically
• PodDisruptionBudget (PDB): minimum available pods during voluntary disruptions
• Taints and tolerations: reserve nodes for specific workloads — GPU nodes, etc.
• Affinity/Anti-affinity: co-locate or spread pods across nodes/zones
...
• Startup probe: delay liveness check during slow startup — prevents premature restart
• HTTP probe: GET /healthz → 200 — most common — implement in every service
• TCP probe: checks if port accepts connections — for non-HTTP services
• exec probe: run command in container — exit 0 = healthy — flexible but heavier
• Probe tuning: initialDelaySeconds, periodSeconds, failureThreshold — calibrate carefully
...
• Canary: route small % to new version — test in production — progressive rollout
• A/B testing: split traffic by user segment — feature experimentation
• GitOps: ArgoCD or Flux — reconcile cluster state with Git repo — audit trail in git
...
• Traffic management: VirtualService, DestinationRule — canary, retries, timeouts
• Observability: distributed tracing, metrics, access logs — built-in without app changes
• Circuit breaking: outlier detection — eject unhealthy upstream instances automatically
• Load balancing: round-robin, least-request, random — per-service configuration
...
• Schema stitching vs federation: federation (Apollo) — independent subgraphs — better for microservices
• Apollo Federation: each service owns its types — @key directive — entity resolution
• Supergraph: composition of subgraphs — Apollo Router routes queries to subgraphs
• Schema validation: graphql-inspector — breaking change detection in CI/CD
...
• Loaders in context: pass via GraphQL context — { loaders: { user: createUserLoader() } }
• Field-level resolvers: only run when field is requested — lazy evaluation
• Abstract type resolvers: __resolveType — determine concrete type for interface/union
• Computed fields: resolver adds calculated field — never stored — derived from DB data
• Subscription resolvers: subscribe function returns AsyncIterator — pub/sub pattern
• Error handling: UserInputError, ForbiddenError, AuthenticationError — typed GraphQL errors

PERSISTED QUERIES:
• Automatic Persisted Queries (APQ): hash query → send hash → server looks up full query
• Registered operations: pre-register in prod — reject unknown queries — security + analytics
• GET for persisted queries: cache at CDN/edge — massive performance win for read-heavy APIs
• Operation names: always name every query/mutation/subscription — analytics, debugging
...
• @requires: field needs other fields to resolve — declare dependencies explicitly
• @provides: can provide external fields — optimization hint to router
• Entity references: { __typename, id } → router fetches full entity from owning subgraph
• Value types: non-entity shared types — must be identical across all subgraphs
• Interface object (@interfaceObject): share interface type across subgraphs
...
• Devtools middleware: import { devtools } from 'zustand/middleware' — Redux DevTools support
• Persist middleware: localStorage/sessionStorage sync — persist across page refreshes
• immer middleware: mutative syntax for complex state updates — ergonomic nested updates
• Subscriptions: useStore.subscribe — react to state changes without causing renders
• Async actions: async function in store — fetch data, update state — no thunks needed
• Reset: keep initial state reference → spread into state on reset action
...
• Optimistic updates: onMutate → update cache → onError → rollback — snappy UI
• Infinite queries: useInfiniteQuery — getNextPageParam — smooth infinite scroll
• Prefetching: queryClient.prefetchQuery — on hover, on route transition — instant navigation
• queryClient.setQueryData: manually update cache — after WebSocket event, after SSE
• Dependent queries: enabled: !!userId — only run when prerequisite data is available
• Parallel queries: useQueries([...]) — fetch multiple at once with one hook
• Query invalidation: queryClient.invalidateQueries — refetch after mutations
• Mutations: useMutation — isLoading, isError, isSuccess states — onSuccess callback
...
• Derived atoms: selector-like — computed from one or more atoms — memoized
• Async atoms: return Promise — Suspense integration — loading built-in
• Jotai DevTools: inspect atom values, trace subscriptions
• Use case: highly granular state — form fields, editor selections, per-item state
...
• RTK Query: built-in data fetching — auto-generated hooks — cache management
• immer integration: built-in — write mutative reducers safely
• Entity adapter: normalized state — CRUD operations — fast lookups by ID
• Listener middleware: react to dispatched actions — side effects without sagas
...
• vi.fn(): mock function — track calls, set return values, throw errors
• vi.useFakeTimers(): control setTimeout/setInterval — test time-dependent code
• vi.setSystemTime(): set current date — test date-dependent logic
• in-source testing: test inside source files — import.meta.vitest guard
• Coverage: v8 or istanbul — nyc-compatible reports — CI threshold enforcement
• Snapshot testing: toMatchSnapshot — detect unexpected output changes
• Concurrent tests: test.concurrent — faster test suites for independent tests
...
• waitFor: await async UI updates — polls until assertion passes or timeout
• renderHook: test custom hooks in isolation — no component needed
• screen.debug(): print current DOM — useful for debugging failing queries
• Custom render: wrap with providers — ThemeProvider, QueryClientProvider, Router
• MSW (Mock Service Worker): intercept HTTP at network level — realistic API mocking
• Avoid implementation details: don't test internal state or component internals
...
• Visual testing: expect(page).toHaveScreenshot() — catch regressions
• Accessibility testing: axe-playwright — automated a11y audit in E2E
• Trace viewer: record and replay test execution — debug flaky tests
• Parallel execution: built-in sharding — run across multiple CI workers
• Mobile emulation: devices preset — test responsive behavior
• Storage state: save auth state to file — skip login in subsequent tests
...
• Pact broker: store and share contracts between teams
• Use for: microservices — catch API breaking changes before integration tests
• Provider states: setup preconditions for each interaction test
• Pact network model: each consumer–provider pair has its own pact contract
...
• Locust: Python — distributed — good for very high load simulations
• Metrics: p50/p95/p99 latency, RPS, error rate, concurrent users
• Spike test: sudden load increase — test auto-scaling and circuit breakers
• Soak test: sustained load over hours — detect memory leaks, gradual degradation
• Stress test: find breaking point — increase until failure — know your limits
• Smoke test: minimal load after deploy — verify basic functionality before real traffic
...
• Builder: construct complex objects step by step — query builders, report generators
• Prototype: clone existing objects — deep copy for complex state initialization
• Object Pool: reuse expensive objects — DB connections, thread pools, WebGL contexts
...
• Composite: tree structures — file systems, UI hierarchies, organization charts
• Decorator: add behavior without subclassing — middleware chains, HOC, class decorators
• Facade: simplified interface to complex subsystem — simplify third-party library usage
• Flyweight: share state between many similar objects — character rendering, particles
• Proxy: control access to object — lazy loading, caching, access control, logging
...
• Memento: capture and restore state — undo history, game save states, snapshots
• Observer: notify dependents of state change — event emitters, pub/sub, React state
• State: change behavior based on internal state — finite state machines, UI modes
• Strategy: swap algorithms at runtime — sorting, payment methods, compression algorithms
• Template Method: define algorithm skeleton, subclasses fill in steps — report generators
• Visitor: add operations without changing classes — AST traversal, serialization
• Interpreter: language grammar as class hierarchy — query parsers, rule engines
• Null Object: default no-op object — avoid null checks — optional dependencies
...
• Currying: transform f(a,b,c) into f(a)(b)(c) — point-free composition
• Memoization: cache function results by arguments — expensive pure function optimization
• Transducer: composable, stateless transformers — efficient pipeline over collections
• Lens: composable getter/setter for nested state — immutable deep updates
• Point-free style: define functions as composition of other functions — no explicit arguments
...
• Don't use ML when: simple rules work, insufficient data, latency requirements too tight
• Start with heuristics: establish a baseline — ML must beat simple rules to justify complexity
• Data first: no ML without clean, labeled data — garbage in, garbage out
• Model complexity: simplest model that meets accuracy requirement — avoid over-engineering
...
• TensorFlow.js: run ML in browser or Node.js — no Python server needed
• Replicate: hosted model inference API — thousands of models — no GPU needed
• Google Vertex AI / AWS SageMaker: managed training and inference — enterprise scale
• Embeddings: OpenAI, Cohere, or open-source (nomic) — semantic search, RAG, clustering
• Classification: sentiment, spam detection, intent recognition — use fine-tuned transformers
• Vision: object detection, image classification — YOLO, ResNet, CLIP via API
...
• Tree of Thoughts (ToT): explore multiple reasoning paths — better for complex problems
• Self-consistency: generate N answers, take majority vote — improves reliability
• ReAct: Reason + Act — interleave thinking and tool use — agentic tasks
• Constitutional AI: critique + revision loop — self-improve outputs iteratively
• Structured prompts: XML tags, JSON schemas in prompt — guides structured output
• Negative prompting: explicitly state what NOT to do — more reliable constraints
• Role prompting: "You are an expert in..." — shifts response style and depth
...
• Pinecone: managed — serverless tier — good for production RAG
• Weaviate: GraphQL API — built-in vectorizer — hybrid search built-in
• Milvus: cloud-native — designed for billions of vectors — distributed
• Dimension mismatch: model dimensions must match collection dimensions — 1536 (OpenAI), 768, 384
• Similarity metrics: cosine (normalized vectors), dot product (performance), Euclidean (absolute distance)
• HNSW index: hierarchical navigable small world — O(log n) search — trade recall for speed
• IVF index: inverted file index — cluster-based — faster than HNSW at very large scale
...
• QLoRA: quantized LoRA — 4-bit quantization — fine-tune on consumer GPU
• Dataset format: JSONL — {"prompt": "...", "completion": "..."} — minimum 100 examples, 1000+ better
• Evaluation: held-out test set — BLEU, ROUGE for generation — human eval for quality
• Overfitting: model memorizes training data — early stopping, validation loss monitoring
• Catastrophic forgetting: fine-tuned model loses general knowledge — use LoRA to preserve base
• OpenAI fine-tuning API: upload JSONL → start job → use fine-tuned model ID
...
• Don't use when: one party controls all data, performance is critical, no token needed, cost matters
• Most "blockchain" use cases are better served by a database + cryptographic signatures
• Evaluate honestly: does this need decentralization or just a better database?
...
• Gas: execution cost — gasLimit (max willing to pay) × gasPrice — EIP-1559: baseFee + tip
• Block: ~12 second finality — Merge moved Ethereum to Proof of Stake
• ABI: Application Binary Interface — how to encode/decode contract function calls
• Events (Logs): emitted by contracts — cheaper than storage — indexed for efficient lookup
• Solidity: primary smart contract language — statically typed — compile to EVM bytecode
...
• Pausable: emergency stop — pause/unpause — circuit breaker for contracts
• ERC-20: fungible token standard — transfer, approve, allowance
• ERC-721: NFT standard — non-fungible token — tokenURI, ownerOf, safeTransferFrom
• ERC-1155: multi-token standard — fungible + non-fungible in one contract
• Proxy pattern: upgradeable contracts — separate logic from state — OpenZeppelin UUPS/Transparent
• Timelock: governance delay — 48h before changes take effect — community review window
...
• viem: TypeScript-first alternative to ethers.js — strict types, tree-shakable
• WalletConnect: multi-wallet support — QR code pairing — mobile wallets
• MetaMask SDK: connect to MetaMask — desktop and mobile — most common wallet
• IPFS: decentralized file storage — NFT metadata, images — Pinata or Infura for pinning
• The Graph: query blockchain data with GraphQL — index events — powerful data layer
...
• Access control: onlyOwner, role-based — every sensitive function must check caller
• Frontrunning: miners see transactions in mempool — commit-reveal for sensitive operations
• Timestamp manipulation: block.timestamp ±15 seconds — don't use for critical timing
• Randomness: block.prevrandao is predictable — use Chainlink VRF for fair randomness
• Slippage: DEX trades — set min amount out — protect users from sandwich attacks
• Audit: always get external audit before mainnet launch — Immunefi bug bounty after
...
• Changesets: version management + changelog generation — for npm package monorepos
• pnpm workspaces: efficient linking — symlinks between packages — single node_modules
• Lerna: package publishing and versioning — often combined with Nx or Turborepo
• Remote caching: Turborepo Remote Cache or Nx Cloud — share cache across CI machines
• Task parallelism: run independent tasks simultaneously — dramatically faster pipelines
• Affected commands: only run tasks for changed packages and their dependents
...
• eslint-plugin-import: enforce import order, detect missing packages
• eslint-plugin-jsx-a11y: accessibility rules for JSX — catch common a11y mistakes
• eslint-plugin-react-hooks: enforce hooks rules — exhaustive-deps — prevent stale closures
• Prettier: opinionated formatter — no config debates — just format and move on
• eslint-config-prettier: disable ESLint rules that conflict with Prettier
• Husky: git hooks — pre-commit lint + format — prevent bad code from entering repo
• lint-staged: only lint files staged for commit — fast even in large repos
...
• OpenAPI Generator: client SDKs from OpenAPI spec — multiple languages
• Orval: React Query hooks + Zod from OpenAPI — TypeScript-first — fits pnpm monorepo
• GraphQL Codegen: typed hooks from schema — type-safe queries, mutations, fragments
• Prisma Migrate: auto-generate migration files from schema changes
• Custom scripts: one-time data transforms, scaffolding — keep in scripts/ directory
...
• Launch config: .vscode/launch.json — debug Node.js, attach to running process
• Recommended extensions: .vscode/extensions.json — prompt teammates to install
• EditorConfig: .editorconfig — consistent indentation across editors — tabs vs spaces
• Snippets: .vscode/snippets/ — team snippets for common patterns — rfc for React components
• Path aliases: tsconfig compilerOptions.paths + vite resolve.alias — @/ for src/ etc.
...
• Docusaurus: MDX-based docs site — versioned docs — search — OpenAPI integration
• Swagger UI: interactive REST API explorer — serve from /api/docs in dev
• Compodoc: Angular project documentation — automatically from code
• ADR (Architecture Decision Records): lightweight text files — capture why decisions were made
• Mermaid: diagrams as code — flowcharts, sequence diagrams, ER diagrams — in Markdown
...
• RDS: managed databases — PostgreSQL, MySQL, Aurora — automated backups, read replicas
• Aurora: PostgreSQL/MySQL compatible — faster, auto-scaling storage, serverless option
• ElastiCache: managed Redis/Memcached — cluster mode, automatic failover
• CloudFront: CDN — edge caching — WAF integration — Lambda@Edge for edge compute
• Route 53: DNS — health checks — latency-based routing — failover routing
• SQS: managed queue — standard (at-least-once) and FIFO — dead letter queues
• SNS: pub/sub — fan-out to SQS, Lambda, HTTP endpoints — push notifications
• EventBridge: event bus — rule-based routing — schedule expressions — SaaS integrations
• Secrets Manager: store and rotate secrets — auto-rotation for RDS — SDK integration
• IAM: identity and access management — users, roles, policies — least privilege always
• VPC: virtual private cloud — subnets, security groups, NACLs, NAT gateway
...
• Cloud SQL: managed PostgreSQL/MySQL — private IP, connection proxy for security
• Cloud Storage: object storage — multi-region, lifecycle management, signed URLs
• BigQuery: serverless data warehouse — petabyte scale — columnar — very fast analytics queries
• Firestore: NoSQL document database — real-time sync — offline support — Firebase compatible
• Pub/Sub: global message bus — push and pull — exactly-once delivery option
• Cloud Armor: WAF + DDoS protection — geo-blocking, rate limiting, OWASP rules
• Artifact Registry: container and package registry — integrated with Cloud Build and GKE
• Vertex AI: managed ML — training, endpoints, vector search, Gemini API
...
• Azure SQL / Cosmos DB: managed SQL / NoSQL — Cosmos multi-model (SQL, Mongo, Cassandra APIs)
• Azure Blob Storage: object storage — tiered (hot, cool, archive)
• Azure CDN: content delivery — global PoPs — custom rules engine
• Azure AD: identity platform — OAuth, OIDC, SAML — enterprise SSO — B2C for customers
• Azure Service Bus: enterprise messaging — queues, topics, subscriptions — AMQP protocol
• Azure DevOps: CI/CD pipelines, boards, repos, artifacts — full ALM platform
• Azure OpenAI: OpenAI models hosted in Azure — enterprise SLA — data residency compliance
...
• Cold start mitigation: provisioned concurrency, keep-alive pings, smaller bundle size
• Lambda @ Edge: run code at CloudFront edge — lowest latency for global users
• Step Functions: orchestrate Lambda functions — state machines — built-in retry + error handling
• Event-driven architecture: S3 event → Lambda → SQS → Lambda → DB — decoupled pipeline
• Idempotency: Lambda can be invoked multiple times — ensure idempotent handlers
• Reserved concurrency: limit max concurrent executions — protect downstream systems
...
• Locale: language + region — en-US, fr-FR, zh-CN — BCP 47 tag format
• Translation keys: meaningful hierarchical keys — common.save, errors.required
• Pluralization: different plural forms per language — English has 2, Russian has 3
• ICU Message Format: {count, plural, one {# item} other {# items}} — handles all cases
• RTL support: Arabic, Hebrew, Farsi — logical properties (margin-inline-start vs margin-left)
• Pseudo-localization: fill strings with accented chars in dev — catch untranslatable strings
...
• Paraglide JS: type-safe i18n — compile-time — no runtime overhead
• Namespace splitting: load only current page's translations — reduce initial bundle
• Context provider: wrap app in I18nextProvider — access anywhere via hook
• Language detection: browser language, URL param, cookie, localStorage
• Translation management: Crowdin, Lokalise, Phrase — connect to CI for auto-export
...
• Intl.Collator: locale-aware string sorting — essential for alphabetical lists
• Intl.PluralRules: correct plural form — 'one', 'few', 'many', 'other'
• Currency: always use ISO 4217 currency codes — never hardcode symbols — let Intl format
• Time zones: store in UTC, display in user's local TZ — Intl.DateTimeFormat with timeZone option
• Temporal API: modern date/time library — immutable, precise, timezone-aware — replace Date

TIMEZONE HANDLING:
• Store all timestamps in UTC — never store local time in DB
• Convert at the display layer only — never in business logic
• User preference: store user timezone in profile — use for all date displays
• Daylight saving time: let Intl handle it — never manually apply DST offsets
• Date arithmetic: use date-fns or luxon — never raw milliseconds for calendar math
• Meeting scheduling: store in UTC, show in each participant's local time
...
• Metrics: numeric measurements over time — request rate, latency, error rate, saturation
• Traces: request journey across services — where time was spent, which service was slow
...
• Manual spans: tracer.startActiveSpan() — instrument custom business operations
• Attributes: add context to spans — user.id, tenant.id, payment.amount
• Context propagation: W3C TraceContext headers — spans linked across service boundaries
• Exporters: OTLP → Jaeger (traces), Prometheus (metrics), OpenSearch (logs)
• Sampling: trace every request in dev, 1-10% in production — control cost
• Baggage: carry context values across service calls — user locale, experiment ID
...
• Summary: like histogram but pre-computed quantiles — less flexible, cheaper
• PromQL: rate(requests_total[5m]) — increase() — histogram_quantile(0.99, ...)
• Recording rules: pre-compute expensive queries — faster dashboard loading
• Alerting rules: alert when metric crosses threshold — route to PagerDuty/Slack
• Label cardinality: don't use user IDs or request IDs as labels — unbounded cardinality kills Prometheus
• Grafana dashboards: variables, time ranges, annotations — share as JSON — version in git
...
• Context: Sentry.setUser({ id, email }) — Sentry.setTag('tenant', id) — better grouping
• Breadcrumbs: automatic trail of events before error — HTTP requests, console logs, navigation
• Release tracking: tag errors by version — detect regressions in new deploys
• Performance: transaction tracing — spans for DB queries, HTTP calls — p95 by operation
• Source maps: upload source maps in CI — human-readable stack traces in production
• Alerts: notify on new errors, regression, spike — Slack, PagerDuty, email
...
• API checks: HTTP request + assertion — status code, response time, body content
• Multi-region: run checks from multiple locations — catch regional outages
• Alerting: PagerDuty, OpsGenie, Slack — on-call rotation — escalation policies
• Status page: Statuspage.io, Incident.io — public communication during incidents
...
• Incident channel: dedicated Slack channel per incident — #inc-2024-01-15-login
• Incident commander: single decision-maker — coordinates responders
• Communication log: timeline of events, actions taken, decisions — written in real time
• Customer communication: status page, proactive email for SEV1-2 — transparency builds trust
• RCA (Root Cause Analysis): blameless postmortem — 5 whys — focus on systems, not people
• Action items: specific, assigned, time-bound — prevent recurrence
...
• Don't chain: user.getAddress().getCity().getZipCode() — tell, don't ask
• Create methods that do the work: user.getZipCode() — hides internal structure

COUPLING & COHESION:
• Low coupling: modules should know as little as possible about each other
• High cohesion: things that change together should be together — SRP at module level
• Temporal coupling: A must be called before B — avoid — make dependencies explicit
• Content coupling: module A accesses internals of module B — worst coupling — use APIs
• Data coupling: pass only necessary data — not entire objects when you need one field
...
• No side effects: pure functions where possible — same input always same output
• Command-query separation: a function either does something or answers something — never both
• Function arguments: 0 is best, 1 is fine, 2 is acceptable, 3 is questionable, 4+ is wrong
• Don't use output arguments: return values — don't mutate parameter and return void
• Avoid null returns: return null object / empty array / default value — prevent null pointer errors
...
• Data clumps: same data always together → create a class for it — Address, DateRange
• Primitive obsession: use domain types instead of primitives — UserId not string, Money not number
• Switch statements: replace with polymorphism — strategy or visitor pattern
• Parallel inheritance: every time you subclass A, you subclass B — refactor to composition
• Lazy class: class that doesn't do enough to justify its existence — merge or delete
• Speculative generality: unnecessary abstractions for hypothetical futures — YAGNI
• Temporary field: field only set sometimes — extract to a separate class/context
• Message chains: a.getB().getC().getD() — Law of Demeter violation — create delegation
• Middle man: class that just delegates to another — either remove or add value
• Inappropriate intimacy: classes coupled too tightly — separate concerns
• Comments that explain bad code: refactor instead of commenting — code should be self-explanatory
...
• Keep-Alive: persistent connections — reuse TCP for multiple requests — default in HTTP/1.1
• Pipelining: send requests without waiting — disabled by default (head-of-line blocking)
• Content negotiation: Accept, Accept-Language, Accept-Encoding — server selects format
• Conditional requests: ETag + If-None-Match, Last-Modified + If-Modified-Since — 304 Not Modified
• Range requests: Content-Range — resumable downloads, video streaming — 206 Partial Content
• Chunked transfer: Transfer-Encoding: chunked — streaming response without known length
...
• Server push: proactively send resources before client asks — link rel=preload is preferred now
• Stream prioritization: weights and dependencies — hint browser what to load first
• Binary protocol: binary framing layer — more efficient parsing than HTTP/1.1 text
• Requires TLS: all browsers require HTTPS for HTTP/2 — good for security too

HTTP/3 / QUIC:
• UDP-based: QUIC protocol replaces TCP — no head-of-line blocking at transport layer
• 0-RTT resumption: immediate data with reconnect — no handshake delay for known servers
• Connection migration: maintain connection when IP changes — mobile networks, wifi handoff
• Improved loss recovery: per-stream loss recovery — packet loss affects only that stream
• Adoption: Cloudflare, Google, Meta serve over HTTP/3 — growing ecosystem
...
• Surrogate-Control: CDN-specific cache control — overrides Cache-Control for edge
• ETag: fingerprint of response — strong (byte-exact) or weak (semantically equivalent) ETags
• Stale-While-Revalidate: serve stale + background refresh — perceived instant response
• Cache-Control: immutable — tell browser asset never changes — force versioned filenames
• CDN cache invalidation: path-based, tag-based — Cloudflare Cache Rules — avoid instant invalidation limits
...
• Permissions-Policy: camera=(), microphone=(), geolocation=(self) — disable unneeded browser APIs
• Cross-Origin-Opener-Policy: same-origin — isolate browsing context — enable SharedArrayBuffer
• Cross-Origin-Embedder-Policy: require-corp — required for COOP to work
• Cross-Origin-Resource-Policy: same-origin — prevent cross-origin reads of sensitive resources
...
• DOM-based XSS: JS reads from DOM source (location.hash) and writes to sink (innerHTML)
• Prevention: output encoding (context-aware) — HTML, JS, CSS, URL encoding — DOMPurify for rich text
• CSP: strongest XSS defense — strict-dynamic + nonce — block inline scripts
• Trusted Types: browser API — enforce safe DOM manipulation — Chrome + polyfill
• HttpOnly cookie: XSS can't read auth cookies — no document.cookie access
...
• Synchronizer Token Pattern: server-generated token in form — validate on submit
• SameSite cookie: SameSite=Strict — cookie not sent on cross-origin requests — simplest CSRF defense
• Custom headers: X-Requested-With: XMLHttpRequest — CORS preflight blocks cross-origin
• Referer validation: check Origin/Referer header — not reliable alone (can be absent)
...
• Time-based blind: IF(condition, SLEEP(5), 0) — detect injection by response delay
• UNION-based: UNION SELECT null,null,null — extract data alongside legitimate results
• Second-order: injected payload stored, executed later — harder to detect
• Stored procedure injection: even parameterized SPs can be vulnerable — validate inside SP
• NoSQL injection: MongoDB $where clause, $regex — don't pass user input to query operators
• ORM bypass: raw query escape hatches — always parameterize even inside ORM raw()
...
• Defense: allowlist of permitted URLs/domains — never blocklist alone
• Validate URL scheme: only http/https — block file://, gopher://, dict://
• DNS rebinding: hostname resolves to internal IP after validation — re-validate after resolution
• Use isolated network: server doing URL fetches should have no access to internal network

DEPENDENCY SECURITY:
• npm audit: scan for known CVEs — run in CI — fail on high severity
• Snyk: deeper analysis — license compliance — container scanning
• SBOM: Software Bill of Materials — inventory all dependencies — regulatory requirement
• Supply chain attacks: typosquatting, compromised packages, malicious updates — lock dependencies
• lockfile: always commit package-lock.json or pnpm-lock.yaml — deterministic installs
• Pinned versions in CI: exact versions — no floating ^ or ~ in lockfile
• Renovate / Dependabot: automated dependency updates — PRs with changelogs — stay current
...
• Continuity: eye follows smooth lines — align elements on a grid — leads eye through layout
• Closure: mind completes incomplete shapes — use incomplete borders for cards
• Figure/Ground: foreground vs background separation — contrast, shadows, overlays for modals
• Common fate: elements moving together are related — animate related items together
...
• Chunking: group related info — phone: (555) 123-4567 vs 5551234567 — easier to process
• Default values: pre-fill forms with sensible defaults — reduce decision fatigue
• Confirmation patterns: destructive actions need confirmation — but not every action
• Error prevention: disable or gray out invalid options — better than showing error after submit
• Undo over confirmation: let users undo actions rather than confirming every delete
...
• 7±2 rule: working memory holds 5-9 items — limit nav items, form fields, options
• Hub and spoke: central home → sub-pages → return to hub — mobile-friendly pattern
• Breadcrumbs: show location in hierarchy — especially for deep or complex sites
• Search: essential when >200 items — faceted search for complex catalogs
• Wayfinding: users always know where they are, where they've been, where they can go
...
• Success feedback: checkmark animation, toast notification — confirm action completed
• Error feedback: shake animation, red border, inline message — immediate, specific
• Empty states: helpful, not just "No data" — explain why and what to do next
• Transition timing: 150ms for micro (hover), 250ms for small (dropdown), 350ms for page
• Haptic feedback: mobile — success vibration pattern vs error pattern — tactile confirmation
...
• Weight: 400 regular, 500 medium, 600 semibold, 700 bold — avoid 800+ for body text
• Letter spacing: tighten for large headings (−0.02em), loosen for all-caps labels (+0.05em)
• Line height: 1.2-1.3 for headings, 1.5-1.6 for body, 1.8 for long-form content
• Measure (line length): 45-75 characters — use max-width: 65ch on prose containers
• Vertical rhythm: consistent spacing between text blocks — 1.5× or 2× base line height
...
• Triadic: three equally spaced — vibrant — use sparingly
• HSL: Hue (0-360°), Saturation (0-100%), Lightness (0-100%) — most intuitive for engineers
• Semantic colors: primary, secondary, success, warning, danger, info, neutral
• Tonal palette: one hue, 10 shades (50-900) — Tailwind's approach — mix and match
• Contrast ratio: WCAG AA 4.5:1 normal text, 3:1 large text — check with tooling
• Color blindness: ~8% of males — don't use color alone to convey meaning — add icons/labels
• Dark mode palette: don't invert — reduce saturation, raise lightness for primary accents
...
• ADRs: document decisions with context — build institutional knowledge
• Brown bag sessions: voluntary learning sessions — share knowledge, elevate the team
• Pair programming: multiply knowledge transfer — build relationships — catch issues early
• Code review as teaching: use review to share patterns — explain why, not just fix
• Tech radar: categorize technologies (adopt, trial, assess, hold) — guide team choices
• Blameless culture: failures are system failures — focus on improvement, not punishment
...
• Feedback: SBI model — Situation, Behavior, Impact — specific, immediate, actionable
• Praise publicly, correct privately — public correction is humiliating
• Radical candor: care personally + challenge directly — not ruinous empathy or obnoxious aggression
• Growth: always have a development goal for each team member — invest in their growth
• Psychological safety: people take risks when they feel safe — celebrate failure as learning
...
• Sprint planning: pull from top of prioritized backlog — estimate capacity, not just story points
• Retrospective: what went well, what didn't, action items — close loop from last retro first
• Definition of Done: agreed criteria — tested, reviewed, deployed to staging, documented
• Velocity: use for forecasting — 3-sprint average — never as a performance metric
• Risk management: identify top 5 risks — mitigation plan — revisit weekly on large projects
• Stakeholder updates: weekly written update — progress, decisions needed, risks — brief
...
• Technical screen: take-home or pair — real-world problem — evaluate problem-solving not trivia
• System design: open-ended — depth of knowledge, communication, trade-off reasoning
• Culture/values: specific behavioral questions — STAR format — look for learning mindset
• Structured interviews: same questions, rubric, multiple interviewers — reduce bias
• Debrief: discuss within 24h — each interviewer rates independently before debrief — avoid anchoring
• Offer: competitive comp — no exploding offers — be transparent about equity
...
• Capture: merchant captures held funds — usually same-day or up to 7 days
• Settlement: funds move between banks — T+1 or T+2 depending on network
• Interchange: fee paid by merchant's bank to customer's bank — 1.5-3% of transaction
• Chargeback: customer disputes with bank — merchant loses funds + chargeback fee ($15-100)
• Dispute evidence: transaction receipt, shipping proof, customer communication, ToS agreement
• PCI DSS: Payment Card Industry standard — Level 1-4 based on volume — use Stripe/Braintree to avoid
• Strong Customer Authentication: EU regulation — requires MFA for payments — 3D Secure 2.0
...
• Freemium: free tier converts to paid — focus on activation rate and conversion funnel
• Marketplace: commission on transactions — take rate — both sides of market
• Enterprise: annual contracts, procurement process, legal review, custom SLAs
• Metered billing: measure usage, bill at end of period — requires usage tracking infrastructure
...
• NRR (Net Revenue Retention): revenue from existing customers — >100% = expansion > churn
...
• LTV:CAC ratio: >3:1 is healthy — < 1:1 means you're losing money acquiring customers
• Payback period: months to recover CAC — <12 months for healthy SaaS
• ARPU: Average Revenue Per User — MRR / total customers — track for upsell effectiveness
• Conversion rate: free → paid — typical 2-5% for freemium — measure by cohort
...
• Gross margin: (Revenue - COGS) / Revenue — SaaS should be >70% — AWS/Stripe costs are COGS
• Operating expenses: S&M, R&D, G&A — fixed + variable — headcount is biggest OpEx
• EBITDA: Earnings Before Interest, Taxes, Depreciation, Amortization — operating profitability proxy
• Cash flow: profit ≠ cash — deferred revenue is cash positive, receivables are profit without cash
• Runway: months of cash remaining — cash / monthly burn rate — critical for startups
...
• Functional programming concepts improve object-oriented code — immutability, pure functions
• Financial systems teach reliability engineering — zero tolerance for silent failures
• Distributed systems failures mirror human organizational failures — communication, consensus
• Game design teaches UX — engagement, reward loops, progressive disclosure — fun = easy and meaningful
• Biology teaches architecture — redundancy (immune system), specialization (organs), evolution (iteration)
...
• Writing as thinking: the act of writing forces clarity — write design docs before coding
• Visual communication: a diagram is worth 1000 words of prose — Mermaid, Excalidraw, Figma
• Storytelling: problems with narrative context are more memorable and persuasive
• The pyramid principle: conclusion first, then supporting points — for executive communication
• Hemingway rule: write at 8th grade level for broad audiences — simple words, short sentences
• Active listening: paraphrase back what you heard — confirm understanding before responding
...
• Pre-mortem: imagine failure — what went wrong? — proactively identify risks
• Weighted criteria matrix: list options × criteria — score each — makes tradeoffs explicit
• OODA loop: Observe, Orient, Decide, Act — fast iteration in uncertain environments
• 70% rule: decide when you have 70% of the information you wish you had — avoid analysis paralysis
• Devil's advocate: assign someone to argue against the preferred option — surface blind spots
• Second-order thinking: what happens next? and what happens after that? — avoid unintended consequences
...
• Distinguish signal from noise: which feedback actually indicates a systemic problem?
• Recognize cognitive biases: confirmation bias, sunk cost, availability heuristic — check yourself
• Build mental models: simplified representations of how systems work — update when reality contradicts
• Avoid cargo culting: understand WHY a practice works — don't blindly apply it
• First principles: break down to fundamental truths — reason up from there — Elon's approach
• Inversion: instead of how to succeed, ask how to avoid failure — avoid the obvious traps
...
• ZGC: concurrent compaction — sub-millisecond pauses — use for latency-sensitive services — Java 15+ production
• Memory: heap (objects), metaspace (class metadata), stack (frames per thread), native memory
• Strong references: default — GC cannot collect — use WeakReference for caches
• Thread model: OS threads (platform threads) vs Project Loom virtual threads (Java 21)
• Virtual threads: lightweight — millions possible — block without blocking OS thread — structured concurrency
• Records: immutable data classes — concise syntax — auto-generated equals, hashCode, toString
• Sealed classes: restrict class hierarchy — exhaustive pattern matching — algebraic types
• Pattern matching: instanceof pattern (Java 16+), switch expressions (Java 14+), guarded patterns
• Text blocks: multiline strings — triple quotes — preserves indentation — JSON/SQL friendly
...
• ConcurrentHashMap: thread-safe — lock striping — better than Collections.synchronizedMap
• BlockingQueue: producer-consumer — ArrayBlockingQueue, LinkedBlockingQueue, PriorityBlockingQueue
• CompletableFuture: async pipeline — thenApply, thenCompose, allOf, anyOf — non-blocking chains
• ExecutorService: thread pool — Executors.newFixedThreadPool, newCachedThreadPool, newSingleThreadExecutor
• ScheduledExecutorService: delayed and periodic tasks — safer than Timer
• Semaphore: limit concurrent access — acquire/release — rate limiting, connection pools
• CountDownLatch: wait for N tasks to complete — single use
• CyclicBarrier: N threads wait for each other — reusable — parallel computation coordination
...
• Spring WebFlux: reactive — non-blocking — R2DBC for reactive DB — Reactor (Mono, Flux)
• Spring Actuator: health endpoints, metrics, info — /actuator/health — Prometheus integration
• @Transactional: declarative transactions — propagation levels — rollback rules
• @Cacheable: declarative caching — Redis or in-memory — cache key expressions
• @Scheduled: cron expressions — fixed rate — fixed delay — task scheduling
• Spring Cloud: Eureka (service discovery), Feign (HTTP client), Hystrix/Resilience4j (circuit breaker), Config Server
• @Profile: environment-specific beans — dev/staging/prod — feature toggles
• Profiles: application-{profile}.yml — override default config — activate via SPRING_PROFILES_ACTIVE
...
• Coroutines: lightweight concurrency — suspend functions — structured concurrency — Flow for streams
• Sealed classes: exhaustive when expressions — type-safe state machines
• Companion objects: static-like members — factory methods, constants — not actually static
• Inline functions: reified generics — lambda inlining — zero-overhead abstractions
• Scope functions: let, run, with, apply, also — operate on object in context — reduce boilerplate
• Delegation: by keyword — interface delegation, property delegation — lazy, observable
• Arrow: functional programming — Either, Option, IO — typeclasses for Kotlin
...
• Generics: <T: Comparable> — associated types in protocols — same power as TypeScript generics
• Closures: capturing values from scope — @escaping for async closures — trailing closure syntax
• Error handling: throws, try, catch — typed errors with enum — Result<Success, Failure>
• Concurrency: async/await (Swift 5.5+) — actors for thread safety — structured concurrency with TaskGroup
• Property wrappers: @State, @Binding, @ObservedObject, @EnvironmentObject — SwiftUI state management
• @MainActor: annotate UI-updating code — ensures main thread — replaces DispatchQueue.main.async
• Sendable: mark types safe to share across concurrency domains — Codable for serialization
...
• @Observable macro (Swift 5.9+): simpler — no @Published needed — automatic observation
• View modifiers: .padding(), .background(), .foregroundColor() — chain to compose styling
• Navigation: NavigationStack + NavigationLink — programmatic navigation with value types
• Lists: List { ForEach(items) { item in ... } } — efficient lazy rendering — swipe actions
• Grids: LazyVGrid, LazyHGrid — column definitions — flexible, fixed, adaptive
• Animations: withAnimation { state = newValue } — .animation() modifier — spring, easeIn, easeOut
• GeometryReader: read parent size — responsive layouts — use sparingly (triggers layout pass)
• ViewBuilder: @ViewBuilder — build conditional views — used in HStack, VStack, custom containers
...
• App Store review: 1-3 days — rejection reasons: crashes, broken functionality, guideline violations
• Certificates: development, distribution, push notification — managed in Apple Developer Portal
• Provisioning profiles: combine app ID + certificate + devices — required for device installation
• Capabilities: push notifications, CloudKit, HealthKit — enable in Xcode + Apple Developer Portal
• In-App Purchases: StoreKit 2 — products, purchases, subscriptions — server-side receipt validation
• WidgetKit: home screen and lock screen widgets — SwiftUI — timeline-based updates
...
• Room: SQLite abstraction — @Entity, @Dao, @Database — type-safe queries — coroutine support
• DataStore: replace SharedPreferences — type-safe — coroutines-based — Preferences or Proto
• WorkManager: guaranteed background work — constraints (network, charging) — retry, chaining
• Navigation Component: type-safe navigation — SafeArgs — deep links — back stack management
...
• rememberSaveable: survive config changes — process death recovery
• LazyColumn / LazyRow: efficient lists — only compose visible items — key parameter for stability
• Side effects: LaunchedEffect (coroutine), SideEffect (synchronize with non-Compose code), DisposableEffect (cleanup)
• derivedStateOf: compute derived state — only recompose when result changes — performance
• Modifier: chain to compose appearance and behavior — order matters — padding before background
• CompositionLocal: implicit parameter passing — LocalContext, LocalDensity — custom locals
• AnimatedVisibility, AnimatedContent: built-in animations — enter/exit transitions
...
• App Startup: Initializer API — lazy initialization — improve cold start time
• Strict mode: detect disk/network on main thread — enable in debug builds — fix all violations
• Avoid memory leaks: don't hold Context in static fields — use WeakReference — LeakCanary
• Battery optimization: minimize background work — Doze mode awareness — use JobScheduler/WorkManager
...
• Scalar types: double, float, int32, int64, bool, string, bytes, uint32, sint32 (efficient negative)
• Message nesting: messages inside messages — import other .proto files
• Repeated fields: list — repeated string tags = 4;
• Oneof: exactly one field set — discriminated union — saves memory
• Map fields: map<string, Project> projects = 3; — key-value pairs
• Well-known types: google.protobuf.Timestamp, Duration, Empty, Any, Struct
• Backward compatibility: never change field numbers — add new optional fields — deprecate old ones
• Forward compatibility: old code ignores unknown fields — safe to add fields in new versions
...
• Deadlines: always set deadlines — gRPC propagates deadline context — prevents hanging
• Cancellation: cancel context propagates to server — server should check ctx.Done()
• Interceptors: middleware for gRPC — logging, auth, metrics — server + client side
• Load balancing: client-side (preferred in gRPC) — pick_first, round_robin, grpclb
• Health checking: grpc_health_v1 — standard protocol — Kubernetes liveness probes
• gRPC-Web: gRPC for browsers — Envoy proxy as gateway — not all methods supported
• Reflection: server exposes schema — grpc_cli, Postman/BloomRPC can discover services
• Metadata: headers equivalent — key-value — authorization, request ID, trace context

BUF TOOLCHAIN:
• buf lint: enforce proto style guide — consistent naming, package structure
• buf format: standardize formatting — like prettier for proto files
• buf breaking: detect breaking changes between versions — CI/CD gate
• buf generate: run protoc plugins — configured in buf.gen.yaml — reproducible codegen
• Buf Schema Registry (BSR): publish and discover schemas — versioned — dependency management
...
• gRPC: efficient binary, streaming, strongly typed — best for internal service-to-service
• GraphQL: flexible querying — aggregation gateway — client-driven data fetching
• Service mesh: infrastructure-level communication — mTLS, retries, circuit breaking without code changes
...
• Pub/Sub: one publisher, many subscribers — fan-out — notification systems
• Request-reply over queue: correlation ID matches response to request — async RPC pattern
• Inbox/Outbox pattern: write event to outbox table atomically with business data — relay reads and publishes — guaranteed delivery
• Saga pattern: distributed transactions — choreography (events) or orchestration (coordinator)
...
• Choreography cons: hard to trace the full flow — distributed logic — debugging is complex
• Orchestration: central saga orchestrator sends commands — tracks state — compensates on failure
• Orchestration pros: explicit flow visible in one place — easier to debug and monitor
• Orchestration cons: orchestrator becomes a dependency — can become a bottleneck
• Process manager: stateful orchestrator — stores current step — handles timeouts and retries
• Temporal: workflow-as-code — durable execution — orchestration with full programming language
...
• Consul: service registry + health checking + KV store + service mesh
• Eureka: Netflix OSS — client-side discovery — Spring Cloud integration
• Kubernetes DNS: services addressable by DNS — cluster-native — my-service.namespace.svc.cluster.local
• Istio/Envoy: sidecar intercepts traffic — transparent service discovery — no code changes needed
...
• Bulkhead: isolate resources — separate thread pools per dependency — failure doesn't cascade
• Timeout: always set — prevent hanging requests from blocking threads — propagate through context
• Rate limiting: protect downstream from overload — token bucket or sliding window
• Fallback: return cached data, default value, or degraded response — better than error
• Hedge requests: send duplicate request after timeout — first response wins — reduces tail latency
• Idempotency keys: safe to retry — unique key per operation — server deduplicates
...
• Commands: request to do something — CreateOrder, ProcessPayment — may be rejected
• Queries: request for data — no state change — GetOrderDetails
• Notifications: inform about something — no response expected — EmailSent, SMSSent
• Event vs Command: events are facts (past tense) — commands are intentions (imperative)
...
• Timestamp: ISO 8601 UTC — when the event occurred — not when published
• Aggregate ID: which entity this event is about — order ID, user ID
• Payload: minimal — what changed — don't include everything — consumers fetch if needed
• Metadata: correlation ID, causation ID, user ID, tenant ID — observability
• Schema registry: Confluent Schema Registry — Avro/Protobuf — version evolution rules

CLOUD EVENTS SPEC:
• Standard: CNCF — defines envelope for events — interoperability between systems
• Attributes: id, source, specversion, type — optional: datacontenttype, subject, time
• Adoption: Knative, Azure Event Grid, Google Eventarc — growing ecosystem
• Benefits: vendor-neutral — standard tooling — event routing based on type/source
...
• Projection: subscribe to event stream — rebuild read model — multiple projections from same events
• Snapshotting: store current state at N events — rebuild from snapshot + delta — performance
• Event replay: replay all events — rebuild any projection — fix bugs by replaying with new code
• GDPR: right to erasure — encrypt event payload — delete encryption key to "erase" — crypto-shredding
• Causal ordering: vector clocks or Lamport timestamps — detect out-of-order events
• Consumer groups: multiple consumers process same stream independently — different projections
...
• SLO (Service Level Objective): target for SLI — 99.9% requests < 200ms — aspirational
• SLA (Service Level Agreement): contractual commitment — consequences if breached — SLO with teeth
• Error budget: 100% - SLO = error budget — 99.9% SLO → 8.76h downtime/year budget
• Error budget policy: if budget exhausted → freeze new features — focus on reliability
• Toil: manual, repetitive operational work — automate it — SREs should spend < 50% on toil
• Eliminating toil: every toil task is an automation opportunity — track toil hours
...
• Throughput: requests per second — minimum required to serve load
• Correctness: fraction of valid responses — data integrity — correct results
• Freshness: data staleness — last updated timestamp vs current time
• Coverage: fraction of work completed — batch job coverage — queue drain rate
• Quality: degraded service — search with less features — serving from cache
...
• n+1 redundancy: always capacity for n+1 instances — one can fail with no impact
• Traffic shaping: shed load gracefully — 503 with Retry-After — don't cascade fail
• Autoscaling: CPU-based (simple) + custom metrics (queue depth, RPS) — test scale-down
• Provisioning lead time: cloud = minutes, hardware = weeks — plan accordingly
• Cost modeling: cost per 1M requests — optimize for cost efficiency at scale
...
• Inject failure: kill instances, add latency, consume resources, drop packets
• Observe: does system degrade gracefully? — does it recover automatically?
• Tools: Chaos Monkey (Netflix), LitmusChaos (Kubernetes), Gremlin (commercial), AWS Fault Injection Simulator
• Game days: scheduled chaos exercises — whole team participates — learn together
• Production chaos: only after extensive staging validation — start with small blast radius
• Failure modes: instance failure, AZ failure, region failure, dependency failure, data corruption, clock skew
...
• Actionable alerts: every alert requires human action — if not actionable, remove or reduce severity
• Escalation paths: primary → secondary → manager → vendor — documented and tested
• On-call handoff: verbal + written — what's happening, what's been tried, what to watch
• Post-incident: blameless postmortem — within 48h — timeline, root cause, action items
• Error budget review: monthly — trend → action — product team aware of reliability state
• On-call rotation: fair distribution — sustainable pace — never one person always on call
...
• Memory management: virtual memory — paging — page tables — TLB for translation cache
• Page fault: access to unmapped page — minor (swap in) or major (disk read) — expensive
• mmap: map files into memory — zero-copy — database files, shared memory, IPC
• Copy-on-Write (COW): fork shares pages — only copy when written — efficient process creation
• Signals: SIGTERM (graceful), SIGKILL (force), SIGUSR1/2 (custom) — signal handlers in apps
• File descriptors: everything is a file — sockets, pipes, devices — limit with ulimit
• epoll/kqueue: efficient I/O multiplexing — O(1) event notification — foundation of Node.js, Nginx
...
• Spatial locality: access nearby memory — arrays are cache-friendly — linked lists are not
• Temporal locality: access recently used data — LRU cache exploits this pattern
• NUMA (Non-Uniform Memory Access): multi-CPU servers — local vs remote memory — NUMA-aware allocation
• Memory alignment: CPU prefers aligned access — struct padding — alignas in C/C++
• Stack vs Heap: stack is fast (SP register), heap needs allocator — stack overflow if too deep
...
• Read-Write lock: many readers OR one writer — concurrent reads are safe
• Spinlock: busy-wait loop — no context switch — only for very short critical sections
• Barrier: all threads reach point before any continues — synchronization point
• Deadlock conditions: mutual exclusion + hold-and-wait + no preemption + circular wait — all four must hold
• Deadlock prevention: lock ordering — always acquire locks in same order — eliminates circular wait
• Livelock: processes change state in response to each other — make no progress — harder to detect
• Starvation: thread waits indefinitely — others keep getting priority — fair scheduling prevents it
...
• Memory ordering: relaxed, acquire, release, seq_cst — control visibility across threads
• Happens-before: formal guarantee — write in thread A is visible in thread B
• Memory barriers: prevent compiler + CPU reordering — explicit fence instructions
• Lock-free stack: Treiber stack — CAS on head pointer — simple lock-free structure
• Lock-free queue: Michael-Scott queue — two CAS operations — used in Java ConcurrentLinkedQueue
• When to use: very high contention + very short critical sections — measure first
...
• AES-CBC: older mode — no built-in authentication — add HMAC separately
• ChaCha20-Poly1305: stream cipher — faster on systems without AES hardware — used in TLS 1.3
• Key derivation: PBKDF2, bcrypt, Argon2 — password → encryption key — adds work factor
• IV/Nonce: must be unique per encryption — never reuse with same key — generate randomly
• Key rotation: change encryption keys periodically — re-encrypt existing data — have a plan
...
• Ed25519: Edwards curve — fast, secure — preferred for new systems — SSH keys, JWT
• ECDH / X25519: key exchange — derive shared secret without transmitting it — perfect forward secrecy
• Public key: safe to share — use to encrypt or verify signatures
• Private key: never share — use to decrypt or sign — store in HSM or encrypted at rest
• Key wrapping: encrypt private key with KEK (key encryption key) — HSM stores KEK
...
• BLAKE3: fast — parallel — modern — use when performance matters
• MD5/SHA-1: broken — collision attacks exist — never use for security — only legacy compat
• HMAC: Hash-based MAC — HMAC-SHA256 = standard — keyed hash — authenticate messages
• Merkle trees: hash tree — Bitcoin, certificate transparency, Git — verify large datasets efficiently
• Commitment schemes: hash(secret + nonce) → commit first, reveal later — prevent front-running
...
• X.509: standard certificate format — DER (binary) or PEM (base64) encoding
• CSR: Certificate Signing Request — generate key pair → create CSR → CA signs → get cert
• OCSP: Online Certificate Status Protocol — check if cert is revoked in real time
• CRL: Certificate Revocation List — list of revoked certs — downloaded periodically
• Certificate pinning: pin expected cert/key — detect MITM — risky (update nightmare) — prefer HPKP alternatives
• Self-signed: signed by own key — not trusted by browsers — fine for internal services with mTLS
...
• PKCE: Proof Key for Code Exchange — code_verifier + code_challenge — prevents auth code interception
• Authorization Code flow: redirect → user logs in → code returned → exchange for tokens
• Access token: short-lived (15min) — sent to resource server — bearer token — opaque or JWT
• Refresh token: long-lived (days/weeks) — exchange for new access token — stored securely
• ID token: OIDC addition — JWT — contains user identity claims — for client consumption only
• JWT: Header.Payload.Signature — base64url encoded — verify signature before trusting
• JWT claims: iss (issuer), sub (subject), aud (audience), exp (expiry), iat (issued at)
• Token introspection: check if token is valid and active — for opaque tokens — RFC 7662
• JWKS endpoint: JSON Web Key Set — public keys for JWT verification — cache with TTL
• OIDC scopes: openid (required), profile, email, address, phone — request user data
• Discovery: /.well-known/openid-configuration — metadata endpoint — dynamic client config
...
• Consistency: data always satisfies invariants — constraints, triggers, application logic
• Isolation: concurrent transactions don't see each other's partial state — isolation levels control this
• Durability: committed data survives crashes — WAL (Write-Ahead Log) — fsync to disk

ISOLATION LEVELS:
• READ UNCOMMITTED: read dirty data (not yet committed) — fastest — almost never use
• READ COMMITTED: only read committed data — default in PostgreSQL/Oracle — most apps fine here
• REPEATABLE READ: same row returns same value within transaction — default in MySQL/InnoDB
• SERIALIZABLE: transactions appear to run sequentially — slowest — use for financial calculations
• Anomalies: dirty read, non-repeatable read, phantom read, lost update — each level prevents some
...
• MVCC (Multi-Version Concurrency Control): readers don't block writers — PostgreSQL uses MVCC
• Deadlocks: DB detects and kills one transaction — application must retry
• SELECT FOR UPDATE SKIP LOCKED: queue-like — grab rows without waiting — job queue pattern
• Advisory locks: pg_try_advisory_lock — application-defined locks — not tied to rows/tables
...
• Eventual consistency: accept temporary inconsistency — design around it — idempotent operations
• Idempotency: same operation N times = same result as once — unique keys prevent duplicate processing
• Last writer wins: simple conflict resolution — loses updates — acceptable for some use cases
• CRDTs: Conflict-free Replicated Data Types — math-based merge — no coordination needed
...
• Point-in-Time Recovery (PITR): base backup + WAL replay — restore to any point
• Streaming replication: primary streams WAL to replicas — near-real-time sync
• Logical replication: row-level changes — subscribe to specific tables — cross-version replication
• pg_waldump: inspect WAL records — debug replication issues, understand transaction log
...
• SSL termination: decrypt HTTPS at gateway — HTTP to backends on private network
• Request/response transformation: add/remove headers, transform body format
• Load balancing: distribute across multiple instances of same service
• Circuit breaking: stop routing to failing backends — return cached or error
• Caching: cache responses at the gateway — reduce backend load
• Logging and tracing: centralized access logs — inject trace IDs into requests
• IP allowlisting/blocklisting: block malicious IPs — geographic restrictions
...
• Traefik: cloud-native — automatic service discovery — middleware — great for Kubernetes
• Nginx: battle-tested reverse proxy — Nginx Plus for API gateway features
• Envoy: high-performance proxy — used in Istio — xDS API — programmable
• APISIX: Kubernetes-native — plugin ecosystem — etcd storage — high performance

BACKEND FOR FRONTEND (BFF):
• Pattern: dedicated API for each client type — mobile, web, third-party
• Benefits: tailor response shape — aggregate multiple services — client-specific auth — versioning
• When to use: clients have very different data needs — mobile needs less data — web needs more
• GraphQL as BFF: clients query exactly what they need — less need for separate BFF layer
• Challenges: more services to maintain — code duplication across BFFs — team ownership unclear
...
• Test coverage: are the right things tested? — are edge cases covered?
• Security review: any untrusted input? any privileged operation without auth check?
• Performance: any N+1 queries? any blocking I/O in hot paths? any unbounded memory growth?
• Error handling: are errors handled at the right level? are they informative?
• API design: will this be easy to use correctly? — can it be misused?
• Consistency: does it follow existing patterns? — naming conventions match?
...
• Distinguish blocking from non-blocking: nit: (style, optional), suggestion: (consider but not blocking), blocker: (must fix before merge)
• Ask questions: "I'm curious about this approach — what led you here?" — might be good reason
• Acknowledge tradeoffs: "This works but here's an alternative to consider..."
• Praise good work: "I really like how you handled the error case here" — genuine, specific
• Request changes clearly: don't leave ambiguous — will you approve after change?
• Avoid sycophancy: "Great PR!" adds no value — substantive feedback does
...
• Don't argue in comments: discuss in person/call if there's genuine disagreement — then update PR
• Thank reviewers for catches: they saved you from a bug in production — genuine appreciation
• Take time to understand: if reviewer found a bug, understand WHY it was a bug — learn the principle
• Small PRs get better reviews: reviewers engage more with focused, small changes

PR DESCRIPTION TEMPLATE:
• What: brief summary of what changed
• Why: business/technical reason for the change — link to issue/ticket
• How: key implementation decisions — any tradeoffs made
• Testing: how was this tested — what scenarios were covered
• Screenshots/recordings: UI changes — before/after — helps reviewers understand
• Deployment notes: any migration, config change, or coordination needed
...
• Time blocking: calendar is your todo list — every task has a time slot — realistic planning
• Maker vs Manager schedule: makers need long blocks — managers need 30-min meetings — protect maker time
• Pomodoro: 25 min focus → 5 min break → repeat — useful for hard tasks you're avoiding
• Energy management: know your peak hours — hard problems during peak — admin during low energy
• 2-minute rule: if task takes < 2 min, do it now — don't add to list — avoid small task backlog
...
• Deliberate practice: practice the specific skill you want to improve — not just doing work
• Build projects: apply new knowledge immediately — "learn by doing" — retention is far higher
• Teach others: explaining forces clarity — blog posts, talks, code reviews — the best learning
• Read code: read open-source code at your level + slightly above — understand real-world patterns
• 1-hour rule: spend 1 hour on a problem before asking — builds debugging skills — respect others' time
• Ask smart questions: what you tried, what happened, what you expected — people help more
...
• Custom scripts: automate repetitive tasks — git hooks, project templates, deployment scripts
• CLI muscle memory: commands you type daily — memorize flags — man pages for reference
• Multi-cursor: available in every major editor — simultaneous editing — powerful for patterns
• Snippets: expand abbreviations to boilerplate — rfc → React component — save minutes per file
• fzf: fuzzy file finder — pipe any list — Ctrl+R for history search — Ctrl+T for files
• ripgrep (rg): faster than grep — respects .gitignore — search codebases instantly
• jq: JSON processing in terminal — filter, transform, extract — essential for API debugging
...
• README: quick start, environment setup, how to run tests — onboarding should take < 30 min
• Runbooks: step-by-step ops procedures — linked from alerts — keep updated after each incident
• Changelog: what changed, when, why — CHANGELOG.md — semver — communicate to users
• API docs: JSDoc + OpenAPI — auto-generated where possible — keep examples current
• Diagram when complex: architecture diagrams, sequence diagrams — Mermaid in markdown
• Update on change: stale docs are worse than no docs — if you change code, update docs
...
• Hacktoberfest: October — many maintainers add beginner issues — community event
• Up for grabs: website listing beginner-friendly issues — curated across repos
• Docs and tests: high-value, low-barrier — maintainers always need more coverage
• Bug reports: reproduce + minimal repro case — valuable even without fix
...
• Small, focused PRs: easier to review and merge — one thing per PR
• Follow code style: match project conventions exactly — look at existing code
• Tests: always add tests for your change — maintainers won't merge without
• Commit messages: Conventional Commits or project's style — meaningful messages
• Patience: maintainers are volunteers — response times vary — follow up politely after 2 weeks
• Credit: thank maintainers — acknowledge their work — positive interactions matter
...
• Code of Conduct: set expectations — Contributor Covenant is standard
• Issue templates: guide reporters — bug vs feature — required info fields
• Bot automation: Dependabot, Renovate — automated dependency updates — label bot
• Sustainable: say no gracefully — scope is health — avoid feature creep
• Burnout: real for maintainers — find co-maintainers — archive if inactive
...
• Cursor IDE: AI-native editor — codebase context — multi-file edits — faster than Copilot for complex tasks
• Claude / ChatGPT: complex explanations, design discussions, debugging approaches — conversational
• Code review bots: automated PR review — catch common issues — free reviewer cycles for harder problems
• Test generation: generate unit tests from implementation — coverage boost — verify they're meaningful
• Documentation generation: JSDoc from code — README drafts — then edit for accuracy
• AI limitations: hallucinations — outdated training data — no awareness of your specific codebase
...
• Better performance: lower overhead than WebGL — direct GPU control — explicit memory management
• TypeScript types: @webgpu/types — full type coverage — excellent autocomplete
• Current state: Chrome 113+ — Firefox/Safari maturing — progressive enhancement approach
• Use cases: 3D games, ML inference (transformers.js uses WebGPU), physics simulations, video processing
...
• hx-target: where to put the response — CSS selector — partial page updates
• hx-trigger: what triggers the request — click, change, load, every 1s
• hx-swap: how to swap content — innerHTML (default), outerHTML, beforeend, afterend
• hx-boost: enhance links and forms — works without JavaScript — progressive enhancement
• Use cases: admin UIs, internal tools, forms — not great for complex SPAs
• Philosophy: HTML is enough — avoid JS complexity when not needed
...
• Node compatibility: mostly compatible — drop-in replacement for many apps
• Native TypeScript: runs .ts files without transpile step — no ts-node needed
• Bun.file: fast file I/O — Bun.serve for HTTP — built-in SQLite driver
• Package manager: bun install — fastest available — lockfile format different from pnpm/yarn
• Testing: bun test — Jest-compatible — watch mode — fast execution
...
• URL imports: import from URLs — no node_modules — content-addressed caching
• Deno Deploy: edge runtime — V8 isolates — global deployment — similar to Cloudflare Workers
• Node compatibility: deno node: prefix — most npm packages work — growing compat
• Deno KV: built-in key-value store — globally replicated on Deploy — zero-setup persistence
• Standard library: first-party maintained — comprehensive — no leftpad risk

WEBCONTAINERS:
• Browser-based Node.js: run Node.js entirely in browser — WebAssembly-based — StackBlitz uses this
• Full Node.js API: real npm, real file system, real network (via service workers)
• Zero-latency dev environment: no server round trips — instant preview — education and prototyping
• Use cases: interactive documentation, online IDEs, browser-based coding challenges
...
• Seek to understand first: listen fully before proposing — understand constraints — earn trust
• Frame as problems not solutions: "here's the problem I'm seeing" — invite others to solve together
• Quantify impact: "this bug costs us X users per day" — data beats opinion — be concrete
• Find allies: identify who else shares your concern — coalition > solo advocacy
• Pick battles: not everything is worth fighting for — strategic about where you spend influence
• Demonstrate first: proof of concept > proposal — running code is most persuasive
...
• Timeboxed spikes: unknown territory → 2-day spike → learning → realistic estimate
• Progressive commitment: small steps — validate assumptions early — adjust based on feedback
• Make your reasoning explicit: "I'm assuming X because Y — is that correct?"
• Default to action: some decisions need to be made to learn — 70% info → decide → iterate
• Escalate blockers: ambiguity that blocks progress → escalate immediately — don't silently wait
...
• QA: understand edge cases — discuss testing strategy upfront — not at the end
• DevOps/SRE: involve early — operational concerns from day 1 — monitoring, alerting, rollback
• Data: what metrics do we need? — instrument before launch — define success metrics
• Legal/Security: regulatory requirements — data retention — privacy by design
...
• Know your audience: engineers want technical depth — executives want outcome and risk
• Manage expectations: better to under-promise and over-deliver — never over-commit
• Disagree and commit: voice concerns clearly — but commit fully once decided — no passive resistance
• Executive communication: one slide, one minute — what, so what, now what — no jargon
...
• Code review as teaching: explain principles — link to resources — not just fix the code
• Pair programming: navigate while they drive — reverse periodically — show your thought process
• Safe to fail: normalize mistakes — blameless — failure is data — adjust and try again
• Celebrate milestones: first PR merged, first production deploy, first incident handled
• Career conversations: what do you want? — where do you want to grow? — how can I help?
• Stretch assignments: tasks slightly beyond current comfort zone — with support — accelerates growth
...
• The most dangerous phrase is "it works on my machine" — automate environment parity
• The second system effect: first system is too simple, second is too complex — aim for the third
• Conway's Law: systems reflect the communication structure of the org that built them — design orgs for desired architecture
• Postel's Law: be conservative in what you send, liberal in what you accept — robust API design
• Hyrum's Law: all observable behaviors will be depended on — even bugs — breaking changes are hard
• Chesterton's Fence: understand why something exists before removing it
• Brooks's Law: adding people to a late project makes it later — communication overhead grows quadratically
• Goodhart's Law: when a measure becomes a target, it ceases to be a good measure — don't optimize metrics
...
• Boolean blindness: function(true, false, true) — use descriptive option objects
• Arrow code: deeply nested ifs — use guard clauses + early returns
• Primitive obsession: string for email, int for money — use domain types
• Premature optimization: micro-optimizing before profiling — measure first
• Overfitting to current requirements: code so specific it can't evolve — design for changeability
• NIH (Not Invented Here): rewriting instead of using proven libraries — use what exists
• YOLO deployment: deploy without testing, monitoring, or rollback — always have a rollback plan
• Resume-driven development: choose technology to learn, not to solve the problem
• Big ball of mud: no architecture — everything coupled to everything — technical debt accumulates
• Lava flow: dead code that nobody dares remove — document and delete with tests as safety net
• Boat anchor: unused component kept "just in case" — YAGNI — delete and rely on version control
...
• Inversion of control: framework calls your code — not your code calls framework — Hollywood principle
• Dependency injection: pass dependencies — don't create them internally — testable and flexible
• Interface segregation: many small, focused interfaces — clients implement only what they use
• Program to interfaces: depend on abstractions — concrete implementations are swappable
• Immutability: unchangeable data prevents a whole class of bugs — functional programming insight
• Idempotency: safe to retry — concurrent systems need this — design operations to be retryable
• Fail fast: detect problems early — validate at boundaries — surface errors immediately, not later
• Defense in depth: multiple layers of security — no single point of failure — assume each layer will fail
