import { Router, type Request, type Response } from "express";
  ...
  // CDN map for bare npm specifiers → esm.sh URLs
  const IMPORT_MAP_ENTRIES: Record<string, string> = {
  ...
  const IMPORT_MAP_TAG = `<script type="importmap">\n${JSON.stringify({ imports: IMPORT_MAP_ENTRIES }, null, 2)}\n</script>`;
  const BABEL_TAG = '<script src="https://unpkg.com/@babel/standalone/babel.min.js"></script>';
  ...
  function hasBareImports(code: string): boolean {
    return /(?:from|import)\s+['"][^'".\/h][^'"]*['"]/m.test(code);
  }
  ...
  function buildHtmlWithAssets(
    ...
    const needsImportMap = needsBabel || hasBareImports(allJsCode) || hasBareImports(html);
    ...
    let headInject = "";
    if (needsImportMap) headInject += IMPORT_MAP_TAG + "\n";
    if (needsBabel) headInject += BABEL_TAG + "\n";
    ...
    // If HTML already has <script type="module"> or <script> with JSX, upgrade to Babel
    if (needsBabel) {
      html = html.replace(
    ...
    if (jsFiles.length > 0) {
      const scriptTags = jsFiles
        .map((f) => {
          if (needsBabel) {
            return `<script type="text/babel" data-presets="react,typescript" data-type="module">/* ${f.name} */\n${f.content}</script>`;
          }
  ...
  router.use("/:projectId", async (req: Request, res: Response): Promise<void> => {
    ...
    if (exactMatch) {
      ...
      if (mime.startsWith("text/html")) {
        ...
        const jsFiles = files.filter(
          (f) =>
            (f.language === "javascript" || f.language === "typescript" || f.name.endsWith(".js") || f.name.endsWith(".jsx") || f.name.endsWith(".ts") || f.name.endsWith(".tsx")) &&
            f.id !== exactMatch.id &&
    ...
    const jsOnly = files.filter((f) => f.language === "javascript" || f.language === "typescript" || f.name.endsWith(".js") || f.name.endsWith(".jsx") || f.name.endsWith(".tsx"));
    const cssOnly = files.filter((f) => f.language === "css" || f.name.endsWith(".css"));

    if (jsOnly.length > 0 || cssOnly.length > 0) {
      const allJs = jsOnly.map((f) => f.content).join("\n");
      const needsBabel = hasJsx(allJs);
      const needsImportMap = needsBabel || hasBareImports(allJs);

      let headExtras = "";
      if (needsImportMap) headExtras += IMPORT_MAP_TAG + "\n";
      if (needsBabel) headExtras += BABEL_TAG + "\n";

      const scriptTags = jsOnly
        .map((f) => {
          if (needsBabel) {
            return `<script type="text/babel" data-presets="react,typescript" data-type="module">/* ${f.name} */\n${f.content}</script>`;
          }
