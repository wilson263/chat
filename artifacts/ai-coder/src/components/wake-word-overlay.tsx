...
function TimerBadge({timer,onDone}:{timer:ActiveTimer;onDone:(id:string)=>void}) {
  ...
  useEffect(()=>{
    const id=setInterval(()=>{
      const rem=timer.endsAt-Date.now(); setLeft(rem);
      if(rem<=0&&!firedRef.current){firedRef.current=true;clearInterval(id);const msg=timer.isReminder?`Reminder: ${timer.reminderText}`:`Time's up! ${timer.label.replace(/^[⏱🔔]\s*/,'')} is done.`;speak(msg);onDone(timer.id);}
    },500);
...
export function WakeWordOverlay() {
  ...
  const onAiQuestion=useCallback((q:string)=>{setLocation(`/?wakeCommand=${encodeURIComponent(q)}`);},[setLocation]);

  const handleCommand=useCallback((raw:string)=>{
    const cmd=parseCommand(raw);
    setCmdType(cmd.type);
    if(cmd.type==='stop-speaking'){stopSpeaking();setSpeaking(false);resetToIdle();return;}
    if(cmd.type==='read-last'){if(!lastAiText){speak("There's no previous response to read.",resetToIdle);return;}setSpeaking(true);speak(lastAiText,()=>{setSpeaking(false);resetToIdle();});return;}
    const result=executeCommand(cmd,setLocation,onAiQuestion,()=>lastAiText,()=>{});
    if(result.timer)setTimers(prev=>[...prev,result.timer!]);
    if(result.action)setTimeout(result.action,400);
    if(result.response){setLastResponse(result.response);setSpeaking(true);speak(result.response,()=>{setSpeaking(false);resetToIdle();});}
    else resetToIdle();
  },[setLocation,onAiQuestion,lastAiText]);

  const{state,supported,cancelCommand,resetToIdle}=useWakeWord(handleCommand,profileReady);

  useEffect(()=>{
    const handler=(e:Event)=>{const text=(e as CustomEvent).detail;if(typeof text==='string')setLastAiText(text);};
  ...
  const borderColor=rejected?'hsl(var(--destructive)/.5)':speaking?'hsl(142 70% 50%/.5)':'hsl(var(--primary)/.4)';
  ...
  return(<>
    ...
    {!profileReady&&!dismissed&&(
      <div style={{position:'fixed',bottom:16,right:16,zIndex:9990,background:'hsl(var(--card))',border:'1px solid hsl(var(--primary)/.4)',borderRadius:16,padding:'14px 16px',boxShadow:'0 8px 32px rgba(0,0,0,.35)',maxWidth:300,animation:'zbslide .3s ease'}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
          ...
          <div><div style={{fontSize:13,fontWeight:700,color:'hsl(var(--foreground))'}}>Set up your voice</div><div style={{fontSize:11,color:'hsl(var(--muted-foreground))'}}>Required for "Hey Zorbix" to work</div></div>
          <button onClick={()=>setDismissed(true)} style={{marginLeft:'auto',background:'none',border:'none',cursor:'pointer',color:'hsl(var(--muted-foreground))',padding:4}}><X style={{width:13,height:13}}/></button>
        </div>
    ...
    {timers.length>0&&!active&&(
      <div style={{position:'fixed',bottom:dismissed?16:90,right:16,zIndex:9989,display:'flex',flexDirection:'column',gap:5,maxWidth:260}}>
    ...
    {active&&(
      <div style={{position:'fixed',bottom:24,right:24,zIndex:9999,background:'hsl(var(--card))',border:`1px solid ${borderColor}`,borderRadius:22,padding:'16px 18px',boxShadow:'0 14px 56px rgba(0,0,0,.5)',minWidth:250,maxWidth:320,animation:'zbslide .25s ease'}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <div style={{position:'relative',width:46,height:46,flexShrink:0}}>
            ...
            <div style={{position:'absolute',inset:0,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center'}}>
              {rejected?<ShieldAlert style={{width:20,height:20,color:'hsl(var(--destructive))'}}/> :speaking?<Volume2 style={{width:20,height:20,color:'hsl(142 70% 50%)'}}/> :state==='processing'?<CmdIcon type={cmdType}/> :<Mic style={{width:20,height:20,color:'hsl(var(--primary))'}}/>}
            </div>
          </div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,fontWeight:700,color:'hsl(var(--foreground))'}}>{stateLabel(state)}</div>
            {speaking&&lastResponse?<div style={{fontSize:11,color:'hsl(var(--muted-foreground))',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{lastResponse}</div>
            :state==='rejected'?<div style={{fontSize:11,color:'hsl(var(--destructive)/.8)'}}>Only your voice can wake Zorbix</div>
            :state==='listening-for-command'?<div style={{fontSize:11,color:'hsl(var(--muted-foreground))'}}>Say a command now</div>:null}
          </div>
          {state==='listening-for-command'&&<button onClick={cancelCommand} style={{padding:5,borderRadius:'50%',border:'none',background:'hsl(var(--muted))',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,color:'hsl(var(--muted-foreground))'}} title="Cancel"><X style={{width:14,height:14}}/></button>}
        </div>
        ...
        {timers.length>0&&<div style={{marginTop:12,display:'flex',flexDirection:'column',gap:5}}>{timers.map(t=><TimerBadge key={t.id} timer={t} onDone={removeTimer}/>)}</div>}
        ...
        {state==='listening-for-command'&&(<>
          ...
          {showHints&&<div style={{marginTop:8,display:'flex',flexWrap:'wrap',gap:5}}>
    ...
    {showEnrollment&&<VoiceEnrollmentModal onClose={()=>setShowEnrollment(false)} onEnrolled={()=>{setProfileReady(true);setDismissed(true);}}/>}
  </>);
}
