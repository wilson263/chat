import { useEffect, useRef, useCallback, useState } from 'react';
  ...
  // Tighter phrases — removed short/ambiguous ones that caused false triggers
  const WAKE_PHRASES = [
    'hey zorbix',
    'hey zorb ix',
    'hey zorbicks',
  ...
  export function useWakeWord(onCommand: (text: string) => void, enabled = true) {
    ...
    // Expose a manual trigger so the voice page can activate listening without wake word
    ...
    return { state, supported, cancelCommand, resetToIdle, triggerListening };
  }
